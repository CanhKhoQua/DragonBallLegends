...
public class PlayersPanel extends JPanel {
        ...
        new Thread(() -> {
            ...
            try (Connection conn = getConnection();
                ...
                if (rs.next()) {
                    ...
                    tabItems.addTab("Hòm thư", createItemPanel(mMail, d));
                    ...
                    btnSave.addActionListener(ev -> savePlayerDB(playerId, accountId, inputs, mBody, mBag, mBox, mMail, originalData, d));
                    pBtn.add(btnSave);
    ...
    private void savePlayerDB(int pid, int accountId, Map<String, Component> inputs, DefaultTableModel mBody, DefaultTableModel mBag, DefaultTableModel mBox, DefaultTableModel mMail, Map<String, String> originalData, JDialog d) {
        new Thread(() -> {
            try {
                // 1. UPDATE PLAYER
                ...
                String jsonMail = buildItemJson(mMail, originalData.get("item_mails_box"));
                ...
                String sqlPlayer = "UPDATE player SET name=?, power=?, head=?, data_inventory=?, data_point=?, items_body=?, items_bag=?, items_box=?, item_mails_box=?, pet=? WHERE id=?";
