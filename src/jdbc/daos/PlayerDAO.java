    ...
    public static void updatePlayer(Player player) {
        if (player != null && player.iDMark.isLoadedAllDataPlayer()) {
            ...
            try {
                ...
                //data costume box
                for (Item item : player.inventory.itemsCostumeBox) {
                    JSONArray opt = new JSONArray();
                    if (item.isNotNullItem()) {
                        dataItem.add(item.template.id);
                        dataItem.add(item.quantity);
                ...
                String itemsCostumeBox = dataArray.toJSONString();
                dataArray.clear();
                ...
                dataArray.add((player.itemTime.isUseBuax2DeTu ? (player.itemTime.totalTimeBuax2DeTu - (System.currentTimeMillis() - player.itemTime.lastTimeBuax2DeTu)) : 0)); // 25
                ...
                dataArray.add(player.itemTime.isUsevevang ? Math.max(0, player.itemTime.totalTimeVeVang - (System.currentTimeMillis() - player.itemTime.lastTimevevang)) : 0);
                ...
                dataArray.add((player.itemTime.isUseKilis ? Math.max(0, player.itemTime.timeLengthKilis - (System.currentTimeMillis() - player.itemTime.lastTimeUseKilis)) : 0)); // 34
                ...
                dataArray.add((player.itemTime.isUseLoX2 ? (ItemTime.TIME_30P - (System.currentTimeMillis() - player.itemTime.lastTimeLoX2)) : 0)); // 45
                dataArray.add((player.itemTime.isUseLoX5 ? (ItemTime.TIME_30P - (System.currentTimeMillis() - player.itemTime.lastTimeLoX5)) : 0)); // 46
                dataArray.add((player.itemTime.isUseLoX7 ? (ItemTime.TIME_30P - (System.currentTimeMillis() - player.itemTime.lastTimeLoX7)) : 0)); // 47
                dataArray.add((player.itemTime.isUseLoX10 ? (ItemTime.TIME_30P - (System.currentTimeMillis() - player.itemTime.lastTimeLoX10)) : 0)); // 48
                dataArray.add((player.itemTime.isUseLoX15 ? (ItemTime.TIME_30P - (System.currentTimeMillis() - player.itemTime.lastTimeLoX15)) : 0)); // 49
                ...
                JSONArray costumeCollection = new JSONArray();
                if (player.costumeCollection != null) {
                    costumeCollection.addAll(player.costumeCollection);
                }
                costumeData.put("collection", costumeCollection);
                costumeData.put("display", player.costumeDisplayId);
                costumeData.put("crystalAura", player.crystalAuraId);
                String costumeCollectionData = costumeData.toJSONString();
                ...
                DBConnecter.executeUpdate(query,
                        ...
                        + ")\n"
                        + "2.Thỏi vàng: "
                        + (isAdmin ? 0 : Util.formatNumber(thoiVangBag + thoiVangBox))
                        + " (Bag: "
                        + (isAdmin ? 0 : Util.formatNumber(thoiVangBag))
                        + " - Box: "
                        + (isAdmin ? 0 : Util.formatNumber(thoiVangBox))
                        + ")\n"
                        + "3.Xu NRO: "
                        + (isAdmin ? 0 : Util.formatNumber(xunrobag + xunrobox))
                        + " (Bag: "
                        + (isAdmin ? 0 : Util.formatNumber(xunrobag))
                        + " - Box: "
                        + (isAdmin ? 0 : Util.formatNumber(xunrobox))
                        + ")\n"
                        + "4.Ngọc rồng 3s: "
                        + (isAdmin ? 0 : Util.formatNumber(ngocRong3sBag + ngocRong3sBox))
                        + " (Bag: "
                        + (isAdmin ? 0 : Util.formatNumber(ngocRong3sBag))
                        + " - Box: "
                        + (isAdmin ? 0 : Util.formatNumber(ngocRong3sBox))
                        + ")\n"
                        + "5.Ngọc rồng 4s: "
                        ...
                        player.id);
                try {
                    DBConnecter.executeUpdate("update player set costume_collection = ? where id = ?", costumeCollectionData, player.id);
                } catch (Exception ignored) {
                }
                try {
                    DBConnecter.executeUpdate("update player set items_costume_box = ? where id = ?", itemsCostumeBox, player.id);
                } catch (Exception ignored) {
                }
                if (player.isOffline) {
...
//        }
    ...
    public static boolean subVND2(Player player, int sotien) {
        ...
        try (Connection con = DBConnecter.getConnectionServer();) {
            ...
            ps.setInt(1, sotien);
            ps.setInt(2, session.actived ? 1 : 0);
