    ...
    public static Player login(MySession session, AntiLogin al) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                MySession existingSession = Client.gI().getSessionByUser(session.userId);
...
//                } else
                if (Client.gI().isAdminMode() && !session.isAdmin) {
                    Service.gI().sendThongBaoOK(session, "Server đang ở chế độ Admin, vui lòng quay lại sau.");
    ...
    private static Player loadPlayer(NDVResultSet rs, boolean isOffline) throws Exception {
        ...
        try {
            ...
            try {
                Object parsedCostume = JSONValue.parse(rs.getString("costume_collection"));
                if (parsedCostume instanceof JSONObject costumeObject) {
                    Object collectionObject = costumeObject.get("collection");
                    if (collectionObject instanceof JSONArray collectionArray) {
                        for (Object object : collectionArray) {
                            player.costumeCollection.add(Integer.parseInt(String.valueOf(object)));
                        }
                    }
                    Object displayObject = costumeObject.get("display");
                    if (displayObject != null) {
                        player.costumeDisplayId = Integer.parseInt(String.valueOf(displayObject));
                    }
                    Object crystalAuraObject = costumeObject.get("crystalAura");
                    if (crystalAuraObject != null) {
                        player.crystalAuraId = Integer.parseInt(String.valueOf(crystalAuraObject));
                    }
                }
            } catch (Exception e) {
                player.costumeCollection.clear();
            ...
            for (int i = 0; i < dataArray.size(); i++) {
                ...
                if (tempId != -1) {
                    ...
                    ItemService.gI().normalizeItemOptions(item);
...
//            }
            ...
            //data costume box
            try {
                dataArray = (JSONArray) JSONValue.parse(rs.getString("items_costume_box"));
                for (int i = 0; dataArray != null && i < dataArray.size(); i++) {
                    ...
                    player.inventory.itemsCostumeBox.add(item);
                }
                if (dataArray != null) {
                    dataArray.clear();
                }
            ...
            int timeBanhHop = 0;
            int timeUseNCD = 0;
            long timeUseBuax2DeTu = 0;
            int timeUseKhauTrang = 0;
            long timeUsevevang = 0;
            long timeKilis = 0;
            long timeMaTroi = 0;
            long timeDoiNhi = 0;
            long timeBoXuong = 0;
            ...
            if (dataArray.size() > 25) {
                timeUseBuax2DeTu = Long.parseLong(String.valueOf(dataArray.get(25)));
            }
            if (dataArray.size() > 26) {
                timeUseKhauTrang = Integer.parseInt(String.valueOf(dataArray.get(26)));
            }
            if (dataArray.size() > 27) {
                timeUsevevang = Long.parseLong(String.valueOf(dataArray.get(27)));
                if (timeUsevevang > 0 && timeUsevevang <= 240) {
                    timeUsevevang *= 60_000L;
                }
            }
            ...
            if (dataArray.size() > 34) {
                timeKilis = Long.parseLong(String.valueOf(dataArray.get(34)));
            }
            ...
            if (dataArray.size() > 45) {
                timeLoX2 = Integer.parseInt(String.valueOf(dataArray.get(45)));
            }
            if (dataArray.size() > 46) {
                timeLoX5 = Integer.parseInt(String.valueOf(dataArray.get(46)));
            }
            if (dataArray.size() > 47) {
                timeLoX7 = Integer.parseInt(String.valueOf(dataArray.get(47)));
            }
            if (dataArray.size() > 48) {
                timeLoX10 = Integer.parseInt(String.valueOf(dataArray.get(48)));
            }
            if (dataArray.size() > 49) {
                timeLoX15 = Integer.parseInt(String.valueOf(dataArray.get(49)));
            }
            if (timechuotmap != 0) {
            ...
            player.itemTime.totalTimeBuax2DeTu = timeUseBuax2DeTu > 0 ? timeUseBuax2DeTu : ItemTime.TIME_30P;
            player.itemTime.lastTimeBuax2DeTu = System.currentTimeMillis();
            player.itemTime.lastTimeKhauTrang = System.currentTimeMillis() - (ItemTime.TIME_30P - timeUseKhauTrang);
            player.itemTime.totalTimeVeVang = timeUsevevang > 0 ? timeUsevevang : 0;
            ...
            player.itemTime.timeLengthKilis = timeKilis > 0 ? timeKilis : ItemTime.TIME_KILIS;
            ...
            player.itemTime.lastTimeLoX2 = System.currentTimeMillis() - (ItemTime.TIME_30P - timeLoX2);
            player.itemTime.lastTimeLoX5 = System.currentTimeMillis() - (ItemTime.TIME_30P - timeLoX5);
            player.itemTime.lastTimeLoX7 = System.currentTimeMillis() - (ItemTime.TIME_30P - timeLoX7);
            player.itemTime.lastTimeLoX10 = System.currentTimeMillis() - (ItemTime.TIME_30P - timeLoX10);
            player.itemTime.lastTimeLoX15 = System.currentTimeMillis() - (ItemTime.TIME_30P - timeLoX15);
            ...
            player.itemTime.isUseLoX2 = timeLoX2 != 0;
            player.itemTime.isUseLoX5 = timeLoX5 != 0;
            player.itemTime.isUseLoX7 = timeLoX7 != 0;
            player.itemTime.isUseLoX10 = timeLoX10 != 0;
            player.itemTime.isUseLoX15 = timeLoX15 != 0;
            ...
            for (int i = 0; i < dataArray.size(); i++) {
                ...
                Skill skill;
                if (point != 0) {
                    skill = SkillUtil.createSkill(player, tempId, point);
                } else {
            ...
            if (!petData.isEmpty()) {
                ...
                int minPetBodySize = pet.typePet >= 2 ? 9 : 7;
                while (pet.inventory.itemsBody.size() < minPetBodySize) {
                ...
                for (int i = 0; i < dataArray.size(); i++) {
                    ...
                    Skill skill;
                    if (point != 0) {
                        skill = SkillUtil.createSkill(pet, tempId, point);
                    } else {
                        skill = SkillUtil.createSkillLevel0(tempId);
                    }
                    if (skillTemp.size() > 3) {
            ...
            try {
                ...
                if (dataArray.size() < 2) {
                    ...
                    DailyGiftService.ensureDailyGifts(player);
                }
                dataArray.clear();
            } catch (Exception ex) {
                DailyGiftService.addAndReset(player);
