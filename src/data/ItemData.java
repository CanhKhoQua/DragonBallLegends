...
public class ItemData {
    ...
    private static final int ITEM_TEMPLATE_BATCH_SIZE = 200;
    ...
    public static void updateItem(MySession session) {
        ...
        updateItemTemplateInBatches(session, 750, Manager.ITEM_TEMPLATES.size());
    }
...
//                System.out.println("start: " + start + " -> " + end + " id " + Manager.ITEM_TEMPLATES.get(i).id);
    ...
    private static void updateItemTemplateInBatches(MySession session, int start, int end) {
        for (int from = start; from < end; from += ITEM_TEMPLATE_BATCH_SIZE) {
            int to = Math.min(from + ITEM_TEMPLATE_BATCH_SIZE, end);
            updateItemTemplate(session, from, to);
        }
    }
