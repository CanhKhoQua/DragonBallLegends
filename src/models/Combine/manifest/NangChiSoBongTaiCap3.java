...
public class NangChiSoBongTaiCap3 {
    ...
    public static void showInfoCombine(Player player) {
        ...
        text.append(ConstFont.BOLD_BLUE).append("Tỉ lệ thành công: 75%\n");
        ...
        text.append(player.inventory.ruby >= COST_RUBY ? ConstFont.BOLD_BLUE : ConstFont.BOLD_RED).append("Cần ").append(COST_RUBY).append(" hồng ngọc\n");
        text.append(ConstFont.BOLD_GREEN).append("+2 Chỉ số ngẫu nhiên\n");
        if (player.inventory.ruby < COST_RUBY) {
            CombineService.gI().baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU, text.toString(), "Còn thiếu\n" + Util.numberToMoney(COST_RUBY - player.inventory.ruby) + " hồng ngọc");
            return;
        }
    ...
    public static void nangChiSoBongTai(Player player) {
        ...
        if (bongTai == null || manhHonBongTai == null || daXanhLam == null || player.inventory.ruby < COST_RUBY || daXanhLam.quantity < 2 || manhHonBongTai.quantity < 198) {
            return;
        }
        ...
        if (Util.isTrue(75, 100)) {
            ...
            int option2;
            do {
                option2 = options[Util.nextInt(options.length)];
            } while (option2 == option1);
            int param1 = option1 == 94 || option1 == 14 ? Util.nextInt(7, 15) : Util.nextInt(8, 20);
            int param2 = option2 == 94 || option2 == 14 ? Util.nextInt(7, 15) : Util.nextInt(8, 20);
