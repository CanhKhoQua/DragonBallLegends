...
public class CheTaoTrangBiThienSu {
    ...
    public static void showInfoCombine(Player player) {
        ...
        if (cCongThuc == 0) {
            ...
            text.append(player.inventory.gold >= 250_000_000 ? ConstFont.BOLD_BLUE : ConstFont.BOLD_RED).append("Phí nâng cấp: 200 triệu vàng");
            if (manhThienSu.quantity < 999 || player.inventory.gold < 250_000_000) {
        ...
        if (cCongThuc == 1 && cManhThienSu == 1 && cDaNangCap <= 1 && cDaMayMan <= 1) {
            ...
            if (manhThienSu.quantity >= 999 && player.inventory.gold >= 250_000_000) {
                if (Util.isTrue(ratio, 100)) {
                    ...
                    Util.setTimeout(() -> {
                        CombineService.gI().sendAddItemCombine(player, ConstNpc.WHIS,
