...
public class NangCapBongTaiCap3 {
    ...
    public static void showInfoCombine(Player player) {
        ...
        if (quantityManhVo < 20000) {
            CombineService.gI().baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU, text.toString(), "Còn thiếu\n" + (20000 - quantityManhVo) + " Mảnh vỡ bông tai cấp 3");
            return;
        }
    ...
    public static void nangCapBongTai(Player player) {
        ...
        if (Util.isTrue(50, 100)) {
            ...
            InventoryService.gI().subParamItemsBag(player, 1855, 31, 20000);
        } else {
            CombineService.gI().sendEffectFailCombine(player);
            InventoryService.gI().subParamItemsBag(player, 1855, 31, 198);
        }
