    ...
    public static void showInfoCombine(Player player) {
       if (InventoryService.gI().getCountEmptyBag(player) > 0) {
                    if (player.combine.itemsCombine.size() == 2) {
                        ...
                        if (itemkhoagd == null) {
                            CombineService.gI().baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Cần có Item bị khóa giao dịch", "Đóng");
                            return;
                        }
                        ...
                        npcSay += "|2|Sau khi mở khóa Item của bạn sẽ thành Item giao dịch được \n|7|"
                        ...
                        CombineService.gI().baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Cần có Item bị khóa giao dịch và Đá Hoàng Kim", "Đóng");
                    }
                } else {
    ...
    public static void startCombine(Player player) {
       if (player.combine.itemsCombine.size() != 2) {
        ...
        if (player.combine.itemsCombine.stream().filter(item -> item.isNotNullItem() && item.template.id == 1723).count() != 1) {
            Service.gI().sendThongBao(player, "Cần Đá Hoàng Kim mua tại Quy Lão Kame");
            return;
        }
        if (InventoryService.gI().getCountEmptyBag(player) > 0) {
            if (player.inventory.ruby < 2000) {
                Service.gI().sendThongBao(player, "Con cần 2000 hồng ngọc để tẩy...");
                return;
            }
            ...
            if (trangBiKhoagd == null) {
                Service.gI().sendThongBao(player, "Cần Item khóa giao dịch");
                return;
            }
            ...
            if (Util.isTrue(30, 100)) {
                ...
                Service.gI().sendThongBao(player, "Bạn đã mở khóa giao dịch thành công");
                InventoryService.gI().sendItemBag(player);
            } else {
                Service.gI().sendThongBao(player, "Bạn đã mở khóa giao dịch thất bại");
