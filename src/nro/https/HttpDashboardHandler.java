...
public class HttpDashboardHandler implements HttpHandler {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        try {
            ...
            exchange.getResponseHeaders().add("Access-Control-Allow-Headers", "Content-Type, X-Admin-Key");
            ...
            // --- API: ACTIONS (POST) ---
                ...
                if (!isAuthorized(exchange, body)) {
                    statusCode = 401;
                    response = "{\"error\": \"Sai API key quan tri\"}";
                } else {
    ...
    private boolean isAuthorized(HttpExchange exchange, String body) {
        String headerKey = exchange.getRequestHeaders().getFirst("X-Admin-Key");
        if (Manager.apiKey != null && Manager.apiKey.equals(headerKey)) {
            return true;
        }
        Map<String, String> params = parseForm(body);
        return Manager.apiKey != null && Manager.apiKey.equals(params.get("key"));
    }
        ...
        try {
            ...
            switch (action) {
                case "broadcast":
                    ...
                    if (message.isEmpty()) {
                        return "{\"error\": \"Noi dung thong bao trong\"}";
                    }
                    ServerNotify.gI().notify(message);
                    return "{\"msg\": \"Da gui thong bao toi player online\"}";
            ...
            <div class="content">
                ...
                <div id="actions" class="panel">
                    ...
                    <div class="stats-grid">
                        ...
                        <div class="card">
                            <h3>Broadcast</h3>
                            <textarea id="broadcastText" rows="5" style="width:100%; box-sizing:border-box; resize:vertical; margin-bottom:10px" placeholder="Nội dung thông báo trong game..."></textarea>
                            <button class="btn btn-success" style="width:100%" onclick="sendBroadcast()">Gửi thông báo</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <script>
                ...
                async function fetchApi(url, method='GET', body=null) {
                    try {
                        ...
                        if(method === 'POST') opts.headers['X-Admin-Key'] = getAdminKey();
                ...
                function getAdminKey() {
                    let key = localStorage.getItem('nro_admin_key') || '';
                    if(!key) {
                        key = prompt('Nhập API key quản trị');
                        if(key) localStorage.setItem('nro_admin_key', key);
                    }
                    return key;
                }
                ...
                async function sendBroadcast() {
                    let message = document.getElementById('broadcastText').value.trim();
                    if(!message) return alert('Nhập nội dung thông báo');
                    if(!confirm('Gửi thông báo tới tất cả player online?')) return;
                    let d = await fetchApi('/api/action', 'POST', 'action=broadcast&message=' + encodeURIComponent(message));
                    alert(d.msg || d.error);
                    if(d.msg) document.getElementById('broadcastText').value = '';
                }
                ...
                async function postAction(action, param='') {
                    let d = await fetchApi('/api/action', 'POST', 'action=' + encodeURIComponent(action) + '&param=' + encodeURIComponent(param));
                    alert(d.msg || d.error);
                }
