...
//                    break;
                ...
                case 54:
                    if (player != null) {
                        while (_msg.reader().available() > 1) {
                            int rawMobId = _msg.reader().readByte();
                            int masterId = -1;
                            boolean isMobMe = rawMobId == -1;
                            int mobId = rawMobId;
                            if (isMobMe) {
                                if (_msg.reader().available() < 5) {
                                    break;
                                }
                        ...
                        if (player != null && player.isPl()) {
                            ...
                            if (TaskService.gI().getIdTask(player) == ConstTask.TASK_0_0) {
                                if (TaskService.gI().getIdTask(player) == ConstTask.TASK_0_0) {
                                    ...
                                    npcSay += "Mình là " + (player.gender == 0 ? "Puaru" : player.gender == 1 ? "Piano" : "Icarus") + " sẽ đồng hành cùng bạn trên thế giới này\n";
                            ...
                            PlayerService.gI().refreshAuraDelayed(player);
...
//                            player.sendNewPet();
                            ...
                            if (DailyGiftService.checkAndClaimDailyGift(player, ConstDailyGift.THONG_BAO_NGOC_MIEN_PHI)) {
