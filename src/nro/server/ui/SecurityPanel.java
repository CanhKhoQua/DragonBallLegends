...
public class SecurityPanel extends JPanel {
    ...
    private volatile boolean ddosRunning = false;
    ...
    private void initControls() {
        ...
        // Button Auto Scan
        btnAutoScan = new JToggleButton("Packet Capture: OFF");
    ...
    private void initAntiDdosSystem() {
        ...
        // 2. Reset PPS counter mỗi giây, dọn IP không hoạt động quá 60s
        ddosScheduler.scheduleAtFixedRate(() -> {
            ipHitCount.clear();
            long now = System.currentTimeMillis();
            ipLastSeen.entrySet().removeIf(e -> now - e.getValue() > 60000);
        ...
        if (blockedIPs.add(ip)) {
            ...
            try {
                if (!ip.matches("^(\\d{1,3}\\.){3}\\d{1,3}$")) {
                    logDdos("Invalid IP format, skipping block: " + ip);
                    return;
                }
                String[] cmdArr;
                if (System.getProperty("os.name").toLowerCase().contains("win")) {
                    cmdArr = new String[]{"netsh", "advfirewall", "firewall", "add", "rule",
                            "name=Block_" + ip, "dir=in", "action=block", "remoteip=" + ip};
                } else {
                    cmdArr = new String[]{"iptables", "-A", "INPUT", "-s", ip, "-j", "DROP"};
                }
        ...
        if (JOptionPane.showConfirmDialog(this, "Gỡ chặn tất cả IP?", "Confirm", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
            for (String ip : new HashSet<>(blockedIPs)) {
                try {
                    String cmd;
                    if (System.getProperty("os.name").toLowerCase().contains("win")) {
                        String[] delCmd = {"netsh", "advfirewall", "firewall", "delete", "rule",
                                "name=Block_" + ip};
                        Runtime.getRuntime().exec(delCmd);
                    } else {
                        String[] delCmd = {"iptables", "-D", "INPUT", "-s", ip, "-j", "DROP"};
                        Runtime.getRuntime().exec(delCmd);
                    }
    ...
    private void toggleAutoScan() {
        ...
        btnAutoScan.setText("Packet Capture: " + (enabled ? "ON" : "OFF"));
        
        if (enabled) {
            ddosRunning = true;
            startPacketCapture();
            logDdos("Packet capture started.");
        } else {
            if (autoScanFuture != null) autoScanFuture.cancel(true);
            ddosRunning = false;
            logDdos("Packet capture stopped.");
        }
    }
