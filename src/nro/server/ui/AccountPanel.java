...
public class AccountPanel extends JPanel {
    ...
    private void initUI() {
        ...
        // --- Table ---
        String[] columns = {"Head", "ID", "Tài khoản", "Tên NV", "Mật khẩu", "Trạng thái", "VIP", "VND", "Đã Nạp", "Ngày tạo", "IP Login", "Login Cuối"};
    ...
    private void loadData() {
        ...
        updateTable("SELECT a.id, a.username, a.password, a.active, a.ban, a.vip, a.cash, a.danap, a.create_time, a.last_time_login, a.last_login_ip," +
        ...
        new Thread(() -> {
            try (Connection conn = DBConnecter.getConnectionServer(); Statement s = conn.createStatement(); ResultSet rs = s.executeQuery(sql)) {
                while (rs.next()) {
                    ...
                    row.add(ban == 1 ? "?? BAN" : (active == 1 ? "Active" : "Chưa KH"));
                    ...
                    String ip = rs.getString("last_login_ip");
                    row.add(ip != null ? ip : "—");
                    Timestamp tLogin = rs.getTimestamp("last_time_login");
                    row.add(tLogin != null ? DATE_FMT.format(tLogin) : "—");
