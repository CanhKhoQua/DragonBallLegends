...
public class AntiDDoS_BY_Barcoll {
    ...
    private static volatile boolean adaptiveStarted = false;
    ...
    private static void handleClientConnection(Socket clientSocket) {
        ...
        try {
            ...
            forwardConnectionOptimized(clientSocket, clientIp, clientPort);
    ...
    private static void forwardConnectionOptimized(Socket clientSocket, String clientIp, int clientPort) {
        ...
        try {
            ...
            try (InputStream clientInput = clientSocket.getInputStream();
                ...
                final Socket activeBackendSocket = backendSocket;
                Thread clientToBackend = Thread.ofVirtual()
                        .name("AntiDDoS-client-to-backend-" + clientIp)
                        .start(() -> forwardDataAndClose(clientInput, backendOutput, clientSocket, activeBackendSocket, clientIp, "client-to-backend"));
                Thread backendToClient = Thread.ofVirtual()
                        .name("AntiDDoS-backend-to-client-" + clientIp)
                        .start(() -> forwardDataAndClose(backendInput, clientOutput, clientSocket, activeBackendSocket, clientIp, "backend-to-client"));

                clientToBackend.join(SOCKET_TIMEOUT);
                backendToClient.join(SOCKET_TIMEOUT);
            } catch (Exception e) {
            ...
            cleanupConnection(clientIp, clientSocket, true);
            closeSocket(backendSocket);
        }
    }

    private static void forwardDataAndClose(InputStream input, OutputStream output, Socket clientSocket,
                                            Socket backendSocket, String ip, String direction) {
        ...
        maintenanceScheduler.scheduleAtFixedRate(() -> {
            try {
                ...
                if (currentOnline > MAX_ONLINE * 0.8) {
                    // Server đông: thắt chặt giới hạn để chống DDoS
                    if (!adaptiveMode) {
                        adaptiveMode = true;
                        currentMaxPerIp = Math.max(3, (int)(MAX_CONN_PER_IP * 0.5));
                        log("🛡 Adaptive mode ON - Tightened: " + currentMaxPerIp + " per IP");
                    }
