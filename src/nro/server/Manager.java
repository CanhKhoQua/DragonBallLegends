...
public final class Manager {
    ...
    public static String apiKey = System.getenv().getOrDefault("NRO_API_KEY", "CHANGE_ME");
    ...
    public static final Map<Integer, Integer> PART_TYPES = new HashMap<>();
...
//        TamBaoService.loadItem();
    ...
    private void loadParts(Connection con) throws Exception {
        ...
        PART_TYPES.clear();
        try (PreparedStatement ps = con.prepareStatement("select * from part order by id"); ResultSet rs = ps.executeQuery()) {
            while (rs.next()) {
                Part part = new Part();
                part.id = rs.getShort("id");
                part.type = rs.getByte("type");
                PART_TYPES.put(part.id, part.type);
    ...
    private void loadTasks(Connection con) throws SQLException {
        // Task Main
        String sql = "SELECT task_main_template.id, task_main_template.name, detail, task_sub_template.name AS 'sub_name', max_count, notify, npc_id, map FROM task_main_template JOIN task_sub_template ON task_main_template.id = task_sub_template.task_main_id ORDER BY task_main_template.id, task_sub_template.id";
    ...
    public static void reloadNotify() {
        ...
        try (Connection con = DBConnecter.getConnectionServer();
             PreparedStatement ps = con.prepareStatement("select * from notify order by id desc");
             ResultSet rs = ps.executeQuery()) {
            while (rs.next()) {
                loaded.add(rs.getString("name") + "<>" + rs.getString("text"));
            }
            synchronized (NOTIFY) {
                NOTIFY.clear();
                NOTIFY.addAll(loaded);
            }
        } catch (Exception e) {
            Logger.logException(Manager.class, e, "Lỗi reload bảng thông báo");
        }
    }

    private void normalizeGinyuForceTask() {
        for (TaskMain task : TASKS) {
            if (task.id == 22) {
                task.subTasks.clear();
                task.subTasks.add(createSubTask("Tiêu diệt Số 4", -1, "", -1, -1));
                task.subTasks.add(createSubTask("Tiêu diệt Số 3", -1, "", -1, -1));
                task.subTasks.add(createSubTask("Tiêu diệt Số 2", -1, "", -1, -1));
                task.subTasks.add(createSubTask("Tiêu diệt Số 1", -1, "", -1, -1));
                task.subTasks.add(createSubTask("Tiêu diệt Tiểu Đội Trưởng", -1, "", -1, -1));
                task.subTasks.add(createSubTask("Báo cáo với %10", -1, "", -5, -9));
                return;
            }
        }
    }

    private SubTaskMain createSubTask(String name, int maxCount, String notify, int npcId, int mapId) {
        SubTaskMain subTask = new SubTaskMain();
        subTask.name = name;
        subTask.maxCount = (short) maxCount;
        subTask.notify = notify;
        subTask.npcId = (byte) npcId;
        subTask.mapId = (short) mapId;
        return subTask;
    }

    private void loadShopsAndItems(Connection con) throws SQLException {
        // Items Template
        try (PreparedStatement ps = con.prepareStatement("select * from item_template order by id"); ResultSet rs = ps.executeQuery()) {
            while (rs.next()) {
                ...
                while (ITEM_TEMPLATES.size() < item.id) {
                    ITEM_TEMPLATES.add(createMissingItemTemplate(ITEM_TEMPLATES.size()));
                }
    ...
    private ItemTemplate createMissingItemTemplate(int id) {
        ItemTemplate item = new ItemTemplate();
        item.id = (short) id;
        item.type = 27;
        item.gender = 3;
        item.name = "Item dự phòng " + id;
        item.description = "Template dự phòng để giữ đúng index item.";
        item.level = 0;
        item.iconID = 2955;
        item.part = 0;
        item.isUpToUp = false;
