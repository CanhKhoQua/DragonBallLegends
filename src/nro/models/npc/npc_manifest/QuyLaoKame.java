...
public class QuyLaoKame extends Npc {
    ...
    public void confirmMenu(Player player, int select) {
        ...
        if (player.iDMark.getIndexMenu() == ConstNpc.MENU_BOSS_TOWER) {
            handleBossTowerMenu(player, select);
            return;
        }
        if (player.iDMark.getIndexMenu() == ConstNpc.MENU_BOSS_TOWER_REWARD) {
            handleBossTowerRewardMenu(player, select);
            return;
        }
        ...
        switch (player.iDMark.getIndexMenu()) {
            ...
            case ConstNpc.MENU_ACCEPT_GO_TO_BDKB:
                ...
                break;
            case ConstNpc.MENU_BOSS_TOWER:
                handleBossTowerMenu(player, select);
                break;
            case ConstNpc.MENU_BOSS_TOWER_REWARD:
                handleBossTowerRewardMenu(player, select);
                break;
    ...
    private void handleBaseMenu(Player player, int select) {
        ...
        if (chosen.equals("Tháp\nBoss")) {
            this.createOtherMenu(player, ConstNpc.MENU_BOSS_TOWER,
                    "Tháp Boss là thử thách cá nhân theo tuần.\nVượt càng cao, thưởng Xu NRO càng nhiều.",
                    "Vào\nTháp", "Top\nTuần này", "Top\nTuần trước", "Xem\nThưởng", "Nhận\nTop", "Đóng");
            return;
        }
    ...
    private void handleBossTowerMenu(Player player, int select) {
        switch (select) {
            case 0 -> BossTowerService.gI().startChallenge(player, -1);
            case 1 -> BossTowerService.gI().showTop(player);
            case 2 -> BossTowerService.gI().showPreviousTop(player);
            case 3 -> showBossTowerRewardMenu(player, BossTowerService.gI().buildRewardOverviewText(player));
            case 4 -> BossTowerService.gI().claimWeeklyReward(player);
        }
    }

    private void handleBossTowerRewardMenu(Player player, int select) {
        switch (select) {
            case 0 -> showBossTowerRewardMenu(player, BossTowerService.gI().buildRewardOverviewText(player));
            case 1 -> showBossTowerRewardMenu(player, BossTowerService.gI().buildMilestoneRewardText(player));
            case 2 -> showBossTowerRewardMenu(player, BossTowerService.gI().buildPreviousTopRewardText(player));
            case 3 -> showBossTowerRewardMenu(player, BossTowerService.gI().buildRewardRuleText());
            case 4 -> BossTowerService.gI().claimWeeklyReward(player);
        }
    }

    private void showBossTowerRewardMenu(Player player, String text) {
        this.createOtherMenu(player, ConstNpc.MENU_BOSS_TOWER_REWARD, text,
                "Tổng\nQuan", "Mốc\nThưởng", "Top\nTuần trước", "Luật", "Nhận\nTop", "Đóng");
    }
    ...
    private void handleTalk(Player player) {
        if (player.LearnSkill.Time != -1 && player.LearnSkill.Time <= System.currentTimeMillis()) {
            ...
            try {
                ...
                var curSkill = SkillUtil.createSkill(player, SkillUtil.getTempSkillSkillByItemID(player.LearnSkill.ItemTemplateSkillId), level);
    ...
    private void learnSkill(Player player) {
        try {
            String[] subName = ItemService.gI().getTemplate(player.LearnSkill.ItemTemplateSkillId).name.split("");
            byte level = Byte.parseByte(subName[subName.length - 1]);
            Skill curSkill = SkillUtil.createSkill(player, SkillUtil.getTempSkillSkillByItemID(player.LearnSkill.ItemTemplateSkillId), level);
            player.BoughtSkill.add((int) player.LearnSkill.ItemTemplateSkillId);
            SkillUtil.setSkill(player, curSkill);
            var msg = Service.gI().messageSubCommand((byte) 62);
            msg.writer().writeShort(curSkill.skillId);
        ...
        switch (select) {
            case 0 -> { // Hủy kỹ năng đang học
                ...
                try {
                    ...
                    if (curSkill == null) {
                        curSkill = SkillUtil.createSkill(player, SkillUtil.getTempSkillSkillByItemID(skillId), (byte) 1);
                    }
