...
public class OngGohan extends Npc {
    ...
    private void thucHienDiemDanh(Player player) {
        try {
            String checkQuery = "SELECT COUNT(*) FROM history_items_diemdanh WHERE account_id = ? AND item_temp_id = 0 AND DATE(bought_date) = CURDATE()";
            ...
            DBConnecter.executeUpdate("UPDATE account SET DiemDanh = ?, lastDiemDanh = CURDATE() WHERE id = ?", player.getSession().diemdanh, player.getSession().userId);
            ...
            switch (select) {
                ...
                case 2 ->
                    createOtherMenu(player, ConstNpc.MAIL_BOX,
                            ...
                            "Xóa Hết\nHòm Thư", "Đóng");
                case 3 ->
                    createOtherMenu(player, 993, "Mở thành viên 10.000đ hoặc đạt 30 tỷ sức mạnh và hoàn thành xong nhiệm vụ tiêu diệt Fide?",
                    "Mở bằng\n10K VNĐ", "Mở bằng\nSức mạnh", "Đóng");
            }
        }
    }
    ...
    private void showMemberMenu(Player player, int select) {
        ...
        switch (select) {
            ...
            case 1 -> { // Mở bằng sức mạnh
                ...
                if (TaskService.gI().getIdTask(player) < ConstTask.TASK_23_2) {
                    Service.gI().sendThongBao(player, "Yêu cầu hoàn thành nhiệm vụ: Tiêu diệt Fide");
                    return;
                }
