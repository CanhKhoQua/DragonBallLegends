...
//                                    createOtherMenu(player, ConstMenu.MENU_NANG_CAP_TRANG_BI, "Ta sẽ biến trang bị của ngươi thành trang bị Kích Hoạt", "Kích hoạt\nThường", "Kích hoạt\nVIP");
                                ...
                                case 3 -> {
                                    this.createOtherMenu(player, ConstMenu.BUILD_DO_BHM, "|7|\bChi Tiết:\n|0|"
                                            + "\nVĩnh viễn Cải trang: dùng Đá ngũ sắc để xoá hạn sử dụng",
                                            "Vĩnh viễn\nCải trang"
                                    );
                                }
                                case 4 -> {
...
//                                    CombineService.gI().openTabCombine(player, CombineService.TAI_TAO_TRANG_BI_KICH_HOAT);    
                        ...
                        case ConstMenu.BUILD_DO_BHM -> {
                            switch (select) {
                                case 0: //Vĩnh viễn cải trang
                                    CombineService.gI().openTabCombine(player, CombineService.VINH_VIEN_CAI_TRANG);
                                    break;
                            }
                        }
...
//                                    }
                ...
                case 42, 43, 44, 84 -> {
                    if (player.iDMark.isBaseMenu()) {
                        ...
                        switch (player.combine.typeCombine) {
                            case CombineService.NANG_CAP_BONG_TAI, CombineService.NANG_CAP_BONG_TAI_CAP_3, CombineService.NANG_CHI_SO_BONG_TAI, CombineService.NANG_CHI_SO_BONG_TAI_CAP_3, CombineService.LAM_PHEP_NHAP_DA, CombineService.NHAP_NGOC_RONG, CombineService.GIAM_DINH_SACH, CombineService.TAY_SACH, CombineService.NANG_CAP_SACH_TUYET_KY, CombineService.HOI_PHUC_SACH, CombineService.PHAN_RA_SACH -> {
                                if (select == 0) {
                                    CombineService.gI().startCombine(player);
                                }
                            }
