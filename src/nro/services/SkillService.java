...
public class SkillService {
    ...
    public boolean useSkill(Player player, Player plTarget, Mob mobTarget) {
        try {
            ...
            if (player.nPoint == null || player.zone == null
                    || (plTarget != null && (plTarget.nPoint == null || plTarget.zone != player.zone))) {
                return false;
            }
...
//    }
            ...
            if (status == 20) {
                ...
                if (num > 0) {
                    ...
                    for (Mob mobMap : player.newSkill.mobsTaget) {
                        ...
                        msg.writer().writeByte(mobMap.getClientIndex());
                    }
                }
                msg.writer().writeByte(typeItem);
            }
    ...
    public void learSkillSpecial(Player player, byte skillID) {
        Message message = null;
        try {
            Skill curSkill = SkillUtil.createSkill(player, skillID, 1);
    ...
    public void learSkillSpecial(Player player, byte skillID, int level) {
        Message message = null;
        try {
            Skill curSkill = SkillUtil.createSkill(player, skillID, level);
            SkillUtil.setSkill(player, curSkill);
            message = Service.gI().messageSubCommand((byte) 23);
            message.writer().writeShort(curSkill.skillId);
            player.sendMessage(message);
...
//                break;    
    ...
    private void phanSatThuong(Player plAtt, Player plTarget, long dame) {
        if (plAtt != null && plAtt.nPoint != null && plTarget != null && plTarget.nPoint != null) {
            ...
            if (percentPST != 0) {
                ...
                try {
                    ...
                    msg.writer().writeInt((int) Util.maxIntValue(plAtt.nPoint.hp));
    ...
    private void playerAttackPlayer(Player plAtt, Player plInjure, boolean miss) {
        if (plAtt == null || plInjure == null || plAtt.nPoint == null || plInjure.nPoint == null
                || plAtt.zone == null || plInjure.zone != plAtt.zone || plAtt.playerSkill == null
                || plAtt.playerSkill.skillSelect == null || plInjure.effectSkill == null) {
            return;
        }
        ...
        if (plInjure.isBoss && plAtt.nPoint.tlDameAttBoss > 0) {
            dameAttack += dameAttack * plAtt.nPoint.tlDameAttBoss / 100;
        }
        ...
        try {
            ...
            msg.writer().writeInt((int) Util.maxIntValue(dameHit)); //dame ăn
...
//            }
    ...
    public boolean canAttackPlayer2(Player p1, Player p2) {
        ...
        if (p2 instanceof TrainingBoss trainingBoss) {
            return trainingBoss.playerAtt != null && trainingBoss.playerAtt.equals(p1);
        }
