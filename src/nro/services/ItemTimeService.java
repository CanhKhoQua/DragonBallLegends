...
public class ItemTimeService {
    ...
    public void sendAllItemTime(Player player) {
        ...
        if (player.itemTime.isUseBuax2DeTu) {
            sendItemTime(player, 13540, (int) ((player.itemTime.totalTimeBuax2DeTu - (System.currentTimeMillis() - player.itemTime.lastTimeBuax2DeTu)) / 1000));
        }
    ...
    public void sendItemTime(Player player, int itemId, int time) {
        Message msg;
        try {
            if (time > MAX_ITEM_TIME_SECONDS) {
                time = MAX_ITEM_TIME_SECONDS;
            } else if (time < 0) {
                time = 0;
            }
            msg = new Message(-106);
