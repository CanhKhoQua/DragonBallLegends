...
public class PlayerService {
    ...
    public void sendTNSM(Player player, byte type, long param) {
        if (param > 0) {
            ...
            try {
                ...
                msg.writer().writeInt((int) Util.maxIntValue(param));// số tn cần cộng
    ...
    public void sendInfoHpMpMoney(Player player) {
        ...
        try {
            ...
            msg.writer().writeInt((int) Util.maxIntValue(player.nPoint.hp));//chp
            msg.writer().writeInt((int) Util.maxIntValue(player.nPoint.mp));//cmp
    ...
    public void playerMove(Player player, int x, int y) {
        ...
        if (!player.isDie()) {
            ...
            resendCrystalAuraOnMove(player);
            TaskService.gI().checkDoneTaskGoToMap(player, player.zone);
        }
    }

    private void resendCrystalAuraOnMove(Player player) {
        if (!player.isPl() || player.crystalAura() == -1) {
            return;
        }
        if (!Util.canDoWithTime(player.lastTimeResendCrystalAura, 1000)) {
            return;
        }
        player.lastTimeResendCrystalAura = System.currentTimeMillis();
        refreshAura(player);
    }

    public void refreshAura(Player player) {
    ...
    public void refreshAuraDelayed(Player player) {
        if (player == null || !player.isPl()) {
            return;
        }
