...
public class InventoryService {
    ...
    private Item putItemBox(Player player, Item item) {
        switch (player.iDMark.getTypeBox()) {
            ...
            case Inventory.TYPE_COSTUME_BOX -> {
                if (item.template.type != 5 || item.itemOptions.stream().anyMatch(Item.ItemOption::haveExpiryDate)) {
                    Service.gI().sendThongBao(player, "Chỉ có thể cải trang vĩnh viễn vào tủ cải trang!");
                    return item;
                }
                CostumeCollectionService.gI().ensureCostumeBox(player);
                for (int i = 0; i < player.inventory.itemsCostumeBox.size(); i++) {
                    if (!player.inventory.itemsCostumeBox.get(i).isNotNullItem()) {
                        player.inventory.itemsCostumeBox.set(i, item);
                        return ItemService.gI().createItemNull();
                    }
                }
            }
...
//                break;
    ...
    public void itemBoxToBodyOrBag(Player player, int index) {
        if (player.iDMark.getTypeBox() == Inventory.TYPE_COSTUME_BOX) {
            if (index < 0) {
                Service.gI().sendThongBao(player, "Khong the thuc hien");
                return;
            }
            CostumeCollectionService.gI().takeOutCostume(player, index);
            return;
        }
        switch (player.iDMark.getTypeBox()) {
            case Inventory.TYPE_NORMAL_BOX -> {
            ...
            default -> {
                ...
                if (item.isNotNullItem()) {
                    if (player.iDMark.getTypeBox() == Inventory.TYPE_COSTUME_BOX
                            && CostumeCollectionService.gI().selectDisplayFromCollectionBox(player, index)) {
                        sendItemBox(player);
                        return;
                    }
                    boolean done = false;
    ...
    public void sendItemBox(Player player) {
        ...
        try {
            ...
            if (player.iDMark.getTypeBox() == Inventory.TYPE_NORMAL_BOX) {
                ...
                List<Item> boxItems = player.iDMark.getTypeBox() == Inventory.TYPE_COSTUME_BOX
                        ? player.inventory.itemsCostumeBox : player.inventory.itemsBoxCollection;
                if (player.iDMark.getTypeBox() == Inventory.TYPE_COSTUME_BOX) {
                    CostumeCollectionService.gI().ensureCostumeBox(player);
                }
                msg.writer().writeByte(boxItems.size());
                for (Item it : boxItems) {
                    msg.writer().writeShort(it.isNotNullItem() ? it.template.id : -1);
                    if (it.isNotNullItem()) {
                        msg.writer().writeInt(it.quantity);
                        msg.writer().writeUTF(it.getInfo());
    ...
    public void openBox(Player player) {
        ...
        try {
            ...
            msg.writer().writeByte(player.iDMark.getTypeBox() == Inventory.TYPE_COSTUME_BOX ? 2 : 0);
            player.sendMessage(msg);
            msg.cleanup();
            sendItemBox(player);
        } catch (Exception e) {
    ...
    public boolean addItemBox(Player player, Item item) {
        switch (player.iDMark.getTypeBox()) {
            ...
            case Inventory.TYPE_COSTUME_BOX -> {
                ...
                CostumeCollectionService.gI().ensureCostumeBox(player);
                return addItemList(player.inventory.itemsCostumeBox, item);
            }
            default -> {
    ...
    private void handleOption210(Item item) {
        for (int i = 0; i < item.itemOptions.size(); i++) {
            ...
            if (io.optionTemplate.id == 210) {
                ...
                int[] allOptions = {8, 14, 108, 94, 16, 80, 81, 97, 100, 101, 104, 106, 204};
                ...
                for (int option : selectedOptions) {
                    int newParam = 0;
                    if (option == 8 || option == 14 || option == 108 || option == 94) {
                        newParam = 6 + (int) (Math.random() * 3);
                    } else if (option == 16 || option == 80 || option == 81 || option == 97 || option == 100 || option == 101 || option == 104 || option == 204) {
