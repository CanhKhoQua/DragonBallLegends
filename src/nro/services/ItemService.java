...
public class ItemService {
    ...
    public void normalizeItemOptions(Item item) {
        if (item == null || item.template == null || item.itemOptions == null) {
            return;
        }
        if (item.template.id == 1628) {
            item.itemOptions.removeIf(io -> io != null && io.optionTemplate != null && io.optionTemplate.id == 9);
            item.itemOptions.add(new ItemOption(9, 30));
            return;
        }
        if (isTrainArmor(item)) {
            int time = 0;
            for (ItemOption io : item.itemOptions) {
                if (io != null && io.optionTemplate != null && io.optionTemplate.id == 9 && io.param > time) {
                    time = io.param;
                }
            }
            item.itemOptions.removeIf(io -> io != null && io.optionTemplate != null && io.optionTemplate.id == 9);
            item.itemOptions.add(new ItemOption(9, time));
        }
    }

    public void normalizeItemShopOptions(ItemShop itemShop) {
        if (itemShop == null || itemShop.temp == null || itemShop.options == null) {
            return;
        }
        if (itemShop.temp.id == 1628) {
            itemShop.options.removeIf(io -> io != null && io.optionTemplate != null && io.optionTemplate.id == 9);
            itemShop.options.add(new ItemOption(9, 30));
            return;
        }
        if (isTrainArmorId(itemShop.temp.id)) {
            itemShop.options.removeIf(io -> io != null && io.optionTemplate != null && io.optionTemplate.id == 9);
            itemShop.options.add(new ItemOption(9, 0));
        }
    }
    ...
    public Item createItemFromItemMap(ItemMap itemMap) {
        ...
        for (ItemOption option : itemMap.options) {
            item.itemOptions.add(new ItemOption(option));
        }
        return item;
    }
    ...
    private boolean isTrainArmorId(int itemId) {
        return switch (itemId) {
            case 529, 534, 530, 535, 531, 536, 1751 -> true;
            default -> false;
        };
    }
    ...
    public int randTempItemKichHoat(int gender) {
        ...
        int type = Util.nextInt(0, 4); // ao, quan, giay, gang, rada: 20% each
    ...
    public int[] randOptionItemKichHoat(int gender) {
        int[][] options;
        switch (gender) {
            case 0 -> options = new int[][]{
                {128, 140},
                {127, 139},
                {129, 141},
                {250, 253}
            };
            case 1 -> options = new int[][]{
                {130, 142},
                {131, 143},
                {132, 144},
                {251, 254}
            };
            default -> options = new int[][]{
                {134, 137},
                {135, 138},
                {133, 136}
            };
    ...
    public int[] randOptionItemKichHoatBalanced(int gender) {
        int[][] options;
        switch (gender) {
            case 0 -> options = new int[][]{
                {128, 140},
    ...
    public ItemMap randDoTL(Zone zone, int quantity, int x, int y, long id) {
        ...
        initBaseOptionThanLinh(type, itemoptions, -1, true);
    ...
    public void initBaseOptionThanLinh(int type, List<ItemOption> itemoptions, int powerRequire) {
        initBaseOptionThanLinh(type, itemoptions, powerRequire, true);
    }

    public void initBaseOptionThanLinh(int type, List<ItemOption> itemoptions, int powerRequire, boolean addSpecialOption) {
        short[] options = {86, 87};
        ...
        if (addSpecialOption && Util.isTrue(90, 100)) {
            itemoptions.add(new ItemOption(options[Util.nextInt(options.length)], 0));
        }
        itemoptions.add(new ItemOption(21, powerRequire > 0 ? powerRequire : Util.nextInt(15, 40)));
    }

    private void initMaxOptionThanLinhFromBox1703(Item item) {
        if (item == null || item.template == null || item.itemOptions == null) {
            return;
        }
        item.itemOptions.clear();
        initBaseOptionThanLinhMax(item.template.type, item.itemOptions, 20);
        item.content = item.getContent();
        item.info = item.getInfo();
    }

    private void initBaseOptionThanLinhMax(int type, List<ItemOption> itemoptions, int powerRequire) {
        switch (type) {
            case 0 ->
                itemoptions.add(new ItemOption(47, 900 * 115 / 100));
            case 1 -> {
                int chiso = 49000 * 115 / 100;
                itemoptions.add(new ItemOption(22, chiso / 1000));
                itemoptions.add(new ItemOption(27, chiso * 125 / 1000));
            }
            case 2 ->
                itemoptions.add(new ItemOption(0, 4500 * 115 / 100));
            case 3 -> {
                int chiso = 49000 * 115 / 100;
                itemoptions.add(new ItemOption(23, chiso / 1000));
                itemoptions.add(new ItemOption(28, chiso * 125 / 1000));
            ...
            case 4 ->
                itemoptions.add(new ItemOption(14, 17 * 115 / 100));
        }
        itemoptions.add(new ItemOption(21, powerRequire));
    }

    private void initMaxOptionHuyDietFromBox1704(Item item) {
        if (item == null || item.template == null || item.itemOptions == null) {
            return;
        }
        item.itemOptions.clear();
        List<Item.ItemOption> baseOptions = getListOptionItemShop((short) item.template.id);
        if (!baseOptions.isEmpty()) {
            boolean hasPowerRequire = false;
            for (ItemOption option : baseOptions) {
                int optionId = option.optionTemplate.id;
                if (optionId == 30 || optionId == 164) {
                    continue;
                }
                if (optionId == 21) {
                    item.itemOptions.add(new ItemOption(21, 80));
                    hasPowerRequire = true;
                    continue;
                }
                int param = option.param;
                if (InventoryService.gI().optionCanUpgrade(optionId)) {
                    param += param * 15 / 100;
                }
        ...
        switch (item.template.type) {
            case 0 ->
                item.itemOptions.add(new ItemOption(47, 2100 * 115 / 100));
            case 1 ->
                item.itemOptions.add(new ItemOption(22, 80 * 115 / 100));
            case 2 ->
                item.itemOptions.add(new ItemOption(0, 11000 * 115 / 100));
            case 3 ->
                item.itemOptions.add(new ItemOption(23, 82 * 115 / 100));
            case 4 ->
                item.itemOptions.add(new ItemOption(14, 18 * 115 / 100));
        }
        item.itemOptions.add(new ItemOption(21, 80));
        item.content = item.getContent();
        item.info = item.getInfo();
    ...
    public Item otptl(short tempId, int quantity) {
        ...
        if (item.template.type == 0) {
            ...
            item.itemOptions.add(new ItemOption(21, Util.nextInt(15, 40)));
    ...
    public void settlkaio(Player player) {
        for (int i = 0; i < 1; i++) {
            ...
            initMaxOptionThanLinhFromBox1703(ao);
    ...
    public void settlkaioken(Player player) {
        openThanLinhSet(player, new int[]{555, 556, 562, 563, 561}, 250, 253);
    }

    public void settlthanvutrukaio(Player player) {
        openThanLinhSet(player, new int[]{555, 556, 562, 563, 561}, 245, 246, 247, 248);
    }
    ...
    public void settlnail(Player player) {
        openThanLinhSet(player, new int[]{557, 558, 564, 565, 561}, 237, 238, 239, 240);
    }
...
//            Item nhan = ItemService.gI().otptl((short) 561);
    ...
    public void settlcadicm(Player player) {
        openThanLinhSet(player, new int[]{559, 560, 566, 567, 561}, 241, 242, 243, 244);
    }

    private void openThanLinhSet(Player player, int[] itemIds, int... optionIds) {
        Item hop = InventoryService.gI().findItem(player.inventory.itemsBag, 1703);
        if (hop == null) {
            Service.gI().sendThongBao(player, "Không tìm thấy hộp Thần Linh");
            return;
        }
        if (InventoryService.gI().getCountEmptyBag(player) < 1) {
            Service.gI().sendThongBao(player, "Bạn phải có ít nhất 1 ô trống hành trang");
        ...
        int itemId = itemIds[new Random().nextInt(itemIds.length)];
        Item item = ItemService.gI().otptl((short) itemId);
        initMaxOptionThanLinhFromBox1703(item);
        for (int optionId : optionIds) {
            item.itemOptions.add(new Item.ItemOption(optionId, 0));
        }
    ...
    public void sethdkaio(Player player) {
        for (int i = 0; i < 1; i++) {
            ...
            initMaxOptionHuyDietFromBox1704(ao);
            ao.itemOptions.add(new Item.ItemOption(127, 0));
            ao.itemOptions.add(new Item.ItemOption(139, 0));
    ...
    public void sethdkaioken(Player player) {
        openHuyDietSet(player, new int[]{650, 651, 657, 658, 656}, 250, 253);
    }

    public void sethdthanvutrukaio(Player player) {
        openHuyDietSet(player, new int[]{650, 651, 657, 658, 656}, 245, 246, 247, 248);
    }
    ...
    public void sethdnail(Player player) {
        openHuyDietSet(player, new int[]{652, 653, 659, 660, 656}, 237, 238, 239, 240);
    }
    ...
    public void sethdcadicm(Player player) {
        openHuyDietSet(player, new int[]{654, 655, 661, 662, 656}, 241, 242, 243, 244);
    }

    private void openHuyDietSet(Player player, int[] itemIds, int... optionIds) {
        Item hop = InventoryService.gI().findItem(player.inventory.itemsBag, 1704);
        if (hop == null) {
            Service.gI().sendThongBao(player, "Không tìm thấy hộp Hủy Diệt");
            return;
        }
        if (InventoryService.gI().getCountEmptyBag(player) < 1) {
            Service.gI().sendThongBao(player, "Bạn phải có ít nhất 1 ô trống hành trang");
        ...
        int itemId = itemIds[new Random().nextInt(itemIds.length)];
        Item item = ItemService.gI().otphd((short) itemId);
        initMaxOptionHuyDietFromBox1704(item);
        for (int optionId : optionIds) {
            item.itemOptions.add(new Item.ItemOption(optionId, 0));
        }
        item.itemOptions.add(new Item.ItemOption(30, 0));
    ...
    public void OpenSKH(Player player, int itemUseId, int select) throws Exception {
        ...
        if (gender != -1) {
            item = createItemSetKichHoat(items[gender][select], 1);
            if (item != null) {
                item.itemOptions.addAll(ItemService.gI().getListOptionItemShop((short) item.template.id));
                int[] options = randOptionItemKichHoatBalanced(gender);
