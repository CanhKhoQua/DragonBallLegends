...
public class NPoint {
    ...
    public static final byte MAX_LIMIT = 14;
    ...
    private void addOption(ItemOption io) {
        switch (io.optionTemplate.id) {
            ...
            case 19: //Tấn công+#% khi đánh quái
                this.tlDameAttMob.add(io.param);
                break;
            case 204: //Tấn công+#% khi đánh Boss
                this.tlDameAttBoss += io.param;
                break;
            case 22: //HP+#K
                this.hpAdd += io.param * 1000;
                break;
    ...
    private void setDameTrainArmor() {
        if (!this.player.isPet && !this.player.isClone && !this.player.isBoss) {
            ...
            try {
                ...
                if (gtl.isNotNullItem() && ItemService.gI().isTrainArmor(gtl)) {
                    this.wearingTrainArmor = true;
                    this.player.inventory.trainArmor = gtl;
                    this.tlSubSD += ItemService.gI().getPercentTrainArmor(gtl);
                } else {
                    if (this.player.inventory.trainArmor == null) {
                        gtl = this.player.inventory.itemsBag.stream().filter(item -> item.isNotNullItem() && ItemService.gI().isTrainArmor(item) && item.itemOptions != null && item.itemOptions.stream().filter(io -> io.optionTemplate.id == 9 && io.param > 0).findFirst().orElse(null) != null).findFirst().orElse(null);
                        if (gtl == null) {
                            return;
                        }
                        this.player.inventory.trainArmor = gtl;
...
//        }
        ...
        if (this.player.isPet) {
            ...
            // typePet: 1 Mabu, 2 Uub, 3 Kid Beer, 4 Kid Jiren
            Map<Integer, int[]> hpBonusMap = new HashMap<>();
            if (typeFusion == ConstPlayer.HOP_THE_PORATA) {
                hpBonusMap.put(1, new int[]{10});
                hpBonusMap.put(2, new int[]{20});
                hpBonusMap.put(3, new int[]{15});
                hpBonusMap.put(4, new int[]{25});
            } else if (typeFusion == ConstPlayer.HOP_THE_PORATA2) {
                hpBonusMap.put(1, new int[]{10, 15});
                hpBonusMap.put(2, new int[]{20, 25});
                hpBonusMap.put(3, new int[]{15, 20});
                hpBonusMap.put(4, new int[]{25, 30});
            } else if (typeFusion == ConstPlayer.HOP_THE_PORATA3) {
                hpBonusMap.put(1, new int[]{10, 15, 20});
                hpBonusMap.put(2, new int[]{20, 25, 30});
                hpBonusMap.put(3, new int[]{15, 20, 25});
                hpBonusMap.put(4, new int[]{25, 30, 35});
            }
...
//        }
        ...
        if (this.player.isPet) {
            ...
            if (typeFusion == ConstPlayer.HOP_THE_PORATA2) {
                switch (typePet) {
                    case 1 ->
                        bonusMap.put(typePet, 35);
                    case 2 ->
                        bonusMap.put(typePet, 45);
                    case 3 ->
                        bonusMap.put(typePet, 35);
                    case 4 ->
                        bonusMap.put(typePet, 55);
                }
            } else if (typeFusion == ConstPlayer.HOP_THE_PORATA3) {
                switch (typePet) {
                    case 1 ->
                        bonusMap.put(typePet, 65);
                    case 2 ->
                        bonusMap.put(typePet, 75);
                    case 3 ->
                        bonusMap.put(typePet, 60);
                    case 4 ->
                        bonusMap.put(typePet, 90);
                }
            }
...
//        }
    ...
    public long calSucManhTiemNang(long tiemNang) {
        ...
        if (power < getPowerLimit()) {
            ...
            if (this.player.isPet) {
                ...
                tiemNang += getHalfMasterTnsmBonus(master, tn);
    ...
    private long getHalfMasterTnsmBonus(Player master, long baseTnsm) {
        if (master == null) {
            return 0;
        }
        long bonus = 0;
        ...
        if (master.getSession() != null && master.getSession().vip > 0) {
            bonus += baseTnsm * 3;
        }
        if (master.itemTime != null && master.itemTime.isUseDK) {
            bonus += baseTnsm * 2;
        }
        if (master.itemTime != null && master.itemTime.isUseLoX2) {
            bonus += baseTnsm * 2;
        }
        if (master.itemTime != null && master.itemTime.isUseLoX5) {
            bonus += baseTnsm * 5;
        }
        if (master.itemTime != null && master.itemTime.isUseLoX7) {
            bonus += baseTnsm * 7;
        }
        if (master.itemTime != null && master.itemTime.isUseLoX10) {
            bonus += baseTnsm * 10;
        }
        if (master.itemTime != null && master.itemTime.isUseLoX15) {
            bonus += baseTnsm * 15;
        }
        if (master.satellite != null && master.satellite.isIntelligent) {
            bonus += baseTnsm * 10 / 100;
        }
        return bonus / 2;
    }
