...
public class Player implements Runnable {
    ...
    private static final long BADGE_REFRESH_INTERVAL = 4000L;
    ...
    public List<Integer> costumeCollection = new ArrayList<>();
    public int costumeDisplayId = -1;
    public int crystalAuraId = -1;
    public int testAuraId = -1;
    ...
    public long lastTimeResendCrystalAura;
...
//        }
        ...
        updateCSMM();
        if (shouldSendUpdateCountSubTask()) {
    ...
    public long lastTimeMobStateSync;
    private int lastSentTaskMainId = -1;
    private int lastSentTaskIndex = -1;
    private int lastSentTaskCount = -1;

    private boolean shouldSendUpdateCountSubTask() {
        if (playerTask == null || playerTask.taskMain == null
                || playerTask.taskMain.subTasks == null
                || playerTask.taskMain.index < 0
                || playerTask.taskMain.index >= playerTask.taskMain.subTasks.size()) {
            return false;
        }
        task.SubTaskMain subTask = playerTask.taskMain.subTasks.get(playerTask.taskMain.index);
        int taskId = playerTask.taskMain.id;
        int taskIndex = playerTask.taskMain.index;
        int taskCount = subTask.count;
        if (taskId == lastSentTaskMainId && taskIndex == lastSentTaskIndex && taskCount == lastSentTaskCount) {
            return false;
        }
        lastSentTaskMainId = taskId;
        lastSentTaskIndex = taskIndex;
        lastSentTaskCount = taskCount;
        return true;
    }
    ...
    public void autoSendBadges() {
        ...
        if (badges.idBadges != -1 && Util.canDoWithTime(badges.lastTimeSendBadges, BADGE_REFRESH_INTERVAL)) {
            Service.gI().sendBadgesPlayer(this, BADGE_DISPLAY_SECONDS, badges.idBadges);
     ...
     * {1842, 1835, 1836}: ht c2 trái đất
     * {873, 874, 875}: ht c2 namếc
     * {1832, 1835, 1836}: ht c2 xayda
     * {1827, 1830, 1831}: ht c3 trái đất
     * {1837, 1840, 1841}: ht c3 namếc
     * {2225, 2228, 2229}: ht c3 xayda
     */
    private static final short[][] idOutfitFusion = {
        {380, 381, 382}, {383, 384, 385}, {391, 392, 393},
        {870, 871, 872}, {873, 874, 875}, {867, 868, 869},
        {1842, 1835, 1836}, {873, 874, 875}, {1832, 1835, 1836},
        {1827, 1830, 1831}, {1837, 1840, 1841}, {2225, 2228, 2229}};
...
//        }
        ...
        if (this.effectSkill != null) {
            if (this.effectSkill.isBienHinh) {
                int index = this.effectSkill.levelBienHinh - 1;
                if (this.gender >= 0 && this.gender < idAuraSuper.length
                        && index >= 0 && index < idAuraSuper[this.gender].length) {
                    return idAuraSuper[this.gender][index];
                }
            }
            if (this.effectSkill.isSuper) {
                int index = getSuperEvolutionIndex();
                if (this.gender >= 0 && this.gender < idAuraSuper.length && index >= 0 && index < idAuraSuper[this.gender].length) {
                    return idAuraSuper[gender][index];
                }
            }
        }

        byte crystalAura = crystalAura();
        if (crystalAura != -1) {
            return crystalAura;
        }
    ...
    public boolean hasFullCrystalSet(int starRequired) {
        if (this.inventory == null || this.inventory.itemsBody == null || this.inventory.itemsBody.size() < 5) {
            return false;
        }
        for (int i = 0; i <= 4; i++) {
            Item item = this.inventory.itemsBody.get(i);
            if (item == null || !item.isNotNullItem() || item.getOptionParam(102) < starRequired) {
                return false;
            }
        }
        return true;
    ...
    public byte crystalAura() {
        if (hasFullCrystalSet(8) && (crystalAuraId == 28 || crystalAuraId == 63)) {
            return (byte) crystalAuraId;
        }
        return -1;
    }
    ...
    private static final byte[][] idAuraSuper = {
        {31, 31, 13, 12, 11},// Trái đất

        {82, 91, 92, 93, 94},// namec

        {31, 31, 13, 25, 89},// xayda
    };
    ...
    public short getHeadSuper() {
        ...
        switch (gender) {
            case 0:
                return idOutFitSuperEarth[index][0];
            case 1:
                return idOutFitSuperNamec[index][0];
            case 2:
                return idOutFitSuperSaiyan[index][0];
        }
        return -1;
    }
    ...
    public short getBodySuper() {
        ...
        switch (gender) {
            case 0:
                return idOutFitSuperEarth[index][1];
            case 1:
                return idOutFitSuperNamec[index][1];
            case 2:
                return idOutFitSuperSaiyan[index][1];
        }
        return -1;
    }
    ...
    public short getLegSuper() {
        ...
        switch (gender) {
            case 0:
                return idOutFitSuperEarth[index][2];
            case 1:
                return idOutFitSuperNamec[index][2];
            case 2:
                return idOutFitSuperSaiyan[index][2];
        }
        return -1;
    }

    private int getSuperEvolutionIndex() {
        if (gender < 0 || gender > 2) {
            return -1;
        }
        Skill skill = playerSkill == null ? null : playerSkill.getSkillbyId(Skill.BIEN_HINH);
        if (skill == null || skill.point <= 0) {
            return -1;
        }
        int index = (skill.point - 1) - numUseSkill;
        if (index < 0) {
            index = 0;
        }
        int maxIndex = Math.min(idAuraSuper[gender].length, getSuperOutfitLength()) - 1;
        if (maxIndex < 0) {
            return -1;
        }
    ...
    private int getSuperOutfitLength() {
        return switch (gender) {
            case 0 -> idOutFitSuperEarth.length;
            case 1 -> idOutFitSuperNamec.length;
            case 2 -> idOutFitSuperSaiyan.length;
            default -> 0;
        };
    }
    ...
    public short getHead() {
        ...
        Template.ItemTemplate displayCostume = getDisplayCostumeTemplate();
        if (displayCostume != null && displayCostume.head != -1) {
            return (short) displayCostume.head;
        }
    ...
    public short getBody() {
        ...
        Template.ItemTemplate displayCostume = getDisplayCostumeTemplate();
        if (displayCostume != null && displayCostume.body != -1) {
            return (short) displayCostume.body;
        }
    ...
    public short getLeg() {
        ...
        Template.ItemTemplate displayCostume = getDisplayCostumeTemplate();
        if (displayCostume != null && displayCostume.leg != -1) {
            return (short) displayCostume.leg;
        }
    ...
    public Template.ItemTemplate getDisplayCostumeTemplate() {
        if (this.costumeDisplayId < 0) {
            return null;
        }
        Template.ItemTemplate template = ItemService.gI().getTemplate(this.costumeDisplayId);
        if (template == null || template.type != 5) {
            return null;
        }
        boolean inCostumeBox = CostumeCollectionService.gI().hasCostumeInCostumeBox(this, this.costumeDisplayId);
        if (!inCostumeBox) {
            return null;
        }
        return template;
