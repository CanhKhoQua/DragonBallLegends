...
public class ArchivementSanBoss {
        ...
        try (Connection con = DBConnecter.getConnectionServer(); PreparedStatement ps = con.prepareStatement("SELECT id, detail FROM moc_san_boss"); ResultSet rs = ps.executeQuery()) {
            while (rs.next()) {
                ...
                try {
                    ...
                    MOC_SAN_BOSS_CACHE.put(id, dataArray);
                } catch (Exception e) {
                    e.printStackTrace();
                    MOC_SAN_BOSS_CACHE.put(id, new JSONArray());
                }
            }
    ...
    public boolean checkdiemboss(Player pl, int index) {
        if (pl == null || pl.getSession() == null || pl.event == null) return false;
        if (index < 0 || index >= DIEMSANBOSS.length) return false;
        return pl.event.getEventPointBHM() >= DIEMSANBOSS[index];
    }
    public void receiveGem(int index, Player pl) {
        if (pl == null || index < 0 || index >= pl.archivementListDiem.size()) {
            return;
        }
        ...
        if (!checkdiemboss(pl, index)) {
            Service.gI().sendThongBao(pl, "Chưa đủ điểm săn boss để nhận mốc này");
            return;
        }
        if (temp != null) {
            Message msg = null;
    ...
    private void nhanDiemBoss(Player pl, int index) {
        Item item;
        JSONArray dataArray = MOC_SAN_BOSS_CACHE.get(index);
            ...
            try (Connection con2 = DBConnecter.getConnectionServer(); PreparedStatement ps = con2.prepareStatement("SELECT detail FROM moc_san_boss WHERE id = ?")) {
                ...
                try (ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
                        ...
                        if (dataArray != null) {
                            MOC_SAN_BOSS_CACHE.put(index, dataArray);
                        }
                    }
                }
            } catch (SQLException e) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    try {
                        ...
                        if (dataArray.size() != DIEMSANBOSS.length) {
                            if (dataArray.size() < DIEMSANBOSS.length) {
                                for (int j = dataArray.size(); j < DIEMSANBOSS.length; j++) {
                                    dataArray.add(0);
                                }
                            }

                            while (dataArray.size() > DIEMSANBOSS.length) {
                                dataArray.remove(DIEMSANBOSS.length);
                            }

                        }
                        player.archivementListDiem.clear();
                        if (dataArray != null) {
    ...
    public String getNhiemVu4(int index) {
        return index >= 0 && index < DIEMSANBOSS.length ? "" + DIEMSANBOSS[index] : "";
    }

    public String getNhiemVu3(Player player, int index) {
        return player != null && player.event != null ? " " + player.event.getEventPointBHM() : "";
    }
