...
public class ShopService {
    ...
    private static final byte COST_RUBY = 2;
    ...
    public void opendShop(Player player, String tagName, boolean allGender) {
        ...
        try {
            ...
            if ("SANTA_PHUKIEN".equals(shop.tagName)) {
...
//    }
    ...
    private void learnKyNang(Player pl, ItemShop is) {
        ...
        if (!SkillUtil.isSkillItemForPlayer(pl, is.temp.id)) {
            Service.gI().sendThongBao(pl, "Không thể thực hiện");
            return;
        }
        var learnedSkill = SkillUtil.getSkillByItemID(pl, is.temp.id);
        var skillPlayer = learnedSkill != null ? pl.playerSkill.getSkillbyId(learnedSkill.template.id) : null;
        ...
        var skillTemplate = SkillUtil.findSkillTemplate(pl, skillTemplateId);
        var potential = skillTemplate.skillss.stream()
...
//        }
    ...
    private boolean hasUsableBadge(Player pl, int idEffect) {
        if (pl == null || pl.dataBadges == null) {
            return false;
        }
        long now = System.currentTimeMillis();
        for (BadgesData data : pl.dataBadges) {
            if (data.idBadGes == idEffect && (data.timeofUseBadges < 0 || data.timeofUseBadges > now)) {
                return true;
            }
        }
        return false;
    ...
    private void buyItemHD(Player player, int itemTempId) {
        ...
        if (!item.itemOptions.isEmpty()) {
            for (ItemOption ios : item.itemOptions) {
                if (item.template.level == 14 && ios.optionTemplate.id == 21) {
                    itemoptions.add(new ItemOption(21, 80));
