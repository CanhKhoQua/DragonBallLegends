...
public class Boss extends Player implements IBoss, IBossOutfit {
                ...
                if (this.currentLevel == 0) {
                    if (this.parentBoss == null) {
                        ...
                        if (this.isZone01SpawnDisabled && this.zone.map.zones.size() > 2) {
                            ...
                            zoneid = 0;
                            List<Zone> availableZones = new java.util.ArrayList<>();
                            for (int i = zoneid; i < this.zone.map.zones.size(); i++) {
                                Zone zoneCheck = this.zone.map.zones.get(i);
                                if (zoneCheck.getNumOfPlayers() <= 10 && zoneCheck.getBosses().isEmpty()) {
                                    availableZones.add(zoneCheck);
                                }
                            }
                            if (!availableZones.isEmpty()) {
                                this.zone = availableZones.get(Util.nextInt(availableZones.size()));
                            }
                            if (availableZones.isEmpty()) {
                            ...
                            if (zoneid < this.zone.map.zones.size() && availableZones.size() < 0) {
                                this.zone = this.zone.map.zones.get(zoneid);
                            } else {
                                this.changeStatus(BossStatus.REST);
                                this.zone = null;
    ...
    protected void notifyJoinMap() {
        if (canSendNotify()) {
            ServerNotify.gI().notify("BOSS " + getNotifyName() + " vừa xuất hiện tại " + this.zone.map.mapName);
        }
    }

    private boolean canSendNotify() {
        if (this.parentBoss != null
                && this.data[this.currentLevel].getTypeAppear() == AppearType.APPEAR_WITH_ANOTHER) {
            return false;
        }
    ...
    protected String getNotifyName() {
        ...
        for (Boss boss : this.bossAppearTogether[this.currentLevel]) {
            ...
            if (boss.data[nextLevelBoss].getTypeAppear() == AppearType.APPEAR_WITH_ANOTHER) {
                name.append(", ").append(boss.data[nextLevelBoss].getName());
            }
        }
    ...
    public void pointBoss(Player plKill){
        Service.gI().sendThongBao(plKill, "Bạn đã tiêu diệt được " + this.name + " và nhận 1 điểm Săn Boss");
    }
...
//            ChatGlobalService.gI().autoChatGlobal(plKill, "[Thông Báo] " + message);
    ...
    public void reward(Player plKill) {
        ...
        if (plKill.getSession() != null) {
            plKill.getSession().diemboss = plKill.event.getEventPointBHM();
        }
        Service.gI().sendThongBao(plKill, "Bạn đã tiêu diệt được " + this.name + " và nhận 1 điểm Săn Boss");
    }

    private void addBossHuntPoint(Player plKill) {
        if (plKill == null || plKill.event == null) {
            return;
        }
    ...
    public void generalRewards(Player player) { // Hmmmm....phần thưởng chung (boss nào cũng rớt - boss phó bản)
        if (player != null) {
    ...
    protected void dropThanLinhBossRewards(Player player, boolean rollThanLinh) {
        if (player == null || this.zone == null) {
            return;
        }
        int x = this.location.x;
        int y = this.zone.map.yPhysicInTop(x, this.location.y - 24);
        if (rollThanLinh && Util.isTrue(50, 100)) {
            Service.gI().dropItemMap(this.zone, ItemService.gI().randDoTL(this.zone, 1, x, y, player.id));
        }
        Service.gI().dropItemMap(this.zone, new ItemMap(this.zone, 674, 1, x + Util.nextInt(-15, 15), y, player.id));
        if (Util.isTrue(20, 100)) {
            Service.gI().dropItemMap(this.zone, new ItemMap(this.zone, 1705, Util.nextInt(1, 3), x + Util.nextInt(-15, 15), y, player.id));
        }
    }
}
