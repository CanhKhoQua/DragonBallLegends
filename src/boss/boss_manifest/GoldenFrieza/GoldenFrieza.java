...
public class GoldenFrieza extends Boss {
    ...
    public void reward(Player plKill) {
        if (Util.isTrue(50, 100)) {
            ...
            for (int i = 0; i < Util.nextInt(3, 10); i++) {
                Service.gI().dropItemMap(this.zone, new ItemMap(zone, 861, Util.nextInt(10, 20), this.location.x + i * 10, this.zone.map.yPhysicInTop(this.location.x,
                        this.location.y - 24), plKill.id));
            }
        }
    }
                ...
                switch (status) {
                    ...
                    case 1:
                        Boss[] deathBeams = (this.bossAppearTogether != null
                                ...
                                ? this.bossAppearTogether[this.currentLevel]
                                : null;
                        if (deathBeams == null || deathBeams.length == 0) {
                            status = 2;
                            timeChanges = 30000;
                            break;
                        }
