...
public class TopService implements Runnable {
    ...
    private static final long MAY_DAM_REFRESH_MS = 60_000L;
    private static final long TOP_TEMPLATE_REFRESH_MS = 60_000L;
...
//                msg.writer().writeUTF("Điểm săn boss sự kiện : " + Util.numberToMoney(top.daubuoi));
    ...
    @Override
    public void run() {
        long lastTemplateRefresh = 0;
        while (ServerManager.isRunning) {
            try {
                long now = System.currentTimeMillis();
                if (timeRealTop + MAY_DAM_REFRESH_MS < now) {
                    timeRealTop = now;
                    try (Connection con = DBConnecter.getConnectionServer()) {
                        topMayDam = realTop(TOP_MAYDAM, con);
                    } catch (Exception e) {
                        Logger.logException(getClass(), e, "Loi refresh top may dam");
                    }
                }

                if (lastTemplateRefresh + TOP_TEMPLATE_REFRESH_MS < now) {
                    lastTemplateRefresh = now;
                    loadListTop();
                }
                ...
                Logger.logException(getClass(), e, "Loi vong lap TopService");
                try {
                    Thread.sleep(IDLE_SLEEP_MS);
