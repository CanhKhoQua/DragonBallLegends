...
public class UseItem {
    ...
    private static final int ITEM_COSTUME_BOX_DISPLAY = 8;
    ...
    private static final short HOP_CAI_TRANG_HOAI_NIEM = 3001;
    private static final short HOP_PET_VAN_BAY_HOAI_NIEM = 3002;
    private static final short HOP_DEO_LUNG_HOAI_NIEM = 3003;
    private static final short HOP_THE_SUU_TAM = 3015;
    private static final short MANH_RONG_THAN_NAMEC = 1204;
    private static final short RADAR_RONG_THAN_NAMEC = 1142;
    private static final Set<Integer> FUSION_COSTUME_TEMPLATE_IDS = new HashSet<>(
            Arrays.asList(601, 602, 603, 639, 640, 641, 1697, 1859, 1860, 1863, 1864, 2022));
    private static final Set<Integer> BROKEN_COSTUME_MEMORY_TEMPLATE_IDS = new HashSet<>(
            Arrays.asList(1736, 1768));
    private static final Set<Integer> SPECIAL_MEMORY_PET_IDS = new HashSet<>(
            Arrays.asList(1854));
        ...
        try {
            ...
            switch (type) {
                ...
                case ITEM_BODY_PET_TO_BAG:
                    ...
                    break;
                case ITEM_COSTUME_BOX_DISPLAY:
                    if (player.iDMark.getTypeBox() == nro.player.Inventory.TYPE_COSTUME_BOX) {
                        CostumeCollectionService.gI().selectDisplayFromCostumeBox(player, index);
                    }
                    break;
            }
                        ...
                        if (index != -1) {
                            ...
                            if (item.isNotNullItem()) {
                                if (item.template.type == 7
                                        || item.template.id >= 1824 && item.template.id <= 1853) {
                                    msg = new Message(-43);
                        ...
                        switch (item.template.id) {
                            ...
                            case 1809:
                                ...
                                if (!pl.itemTime.isUsevevang) {
                                    ...
                                    long remaining = pl.itemTime.totalTimeVeVang - (now - pl.itemTime.lastTimevevang);
                                    if (remaining < 0) {
                                        remaining = 0;
                                    }
                                    pl.itemTime.lastTimevevang = now;
                                    pl.itemTime.totalTimeVeVang = remaining + addTime;
                                }
                            ...
                            case 194:
                                ...
                                break;
                            default:
                                if (item.template.id >= 1824 && item.template.id <= 1853) {
                                    learnSkill(pl, item);
                                    break;
                                }
                                break;
                            ...
                            case 1453:
                                ...
                                caiTrangHaiTac = ItemService.gI().createNewItem((short) ct);
                                caiTrangHaiTac.itemOptions.add(caiTrangHaiTac.getOptionDaPhaLe());
                                caiTrangHaiTac.itemOptions.add(new Item.ItemOption(87, 0));
                                InventoryService.gI().addItemBag(pl, caiTrangHaiTac);
                                InventoryService.gI().subQuantityItemsBag(pl, item, 1);
                                Service.gI().sendThongBao(pl, "Bạn đã nhận được " + caiTrangHaiTac.template.name);
                                break;
                            ...
                            case 1954:
                                ...
                                break;
                            case HOP_CAI_TRANG_HOAI_NIEM:
                                openCosmeticMemoryBox(pl, item, HOP_CAI_TRANG_HOAI_NIEM);
                                break;
                            case HOP_PET_VAN_BAY_HOAI_NIEM:
                                openCosmeticMemoryBox(pl, item, HOP_PET_VAN_BAY_HOAI_NIEM);
                                break;
                            case HOP_DEO_LUNG_HOAI_NIEM:
                                openCosmeticMemoryBox(pl, item, HOP_DEO_LUNG_HOAI_NIEM);
                                break;
                            case HOP_THE_SUU_TAM:
                                openRadarMemoryBox(pl, item);
                                break;
                            case 1575:
    ...
    private void openCosmeticMemoryBox(Player player, Item box, short boxId) {
        if (InventoryService.gI().getCountEmptyBag(player) <= 0) {
            Service.gI().sendThongBao(player, "Cần 1 ô trống trong hành trang để mở");
            return;
        }
        List<Short> pool = getCosmeticMemoryPool(player, boxId);
        if (pool.isEmpty()) {
            Service.gI().sendThongBao(player, "Hộp này chưa có vật phẩm phù hợp để mở");
            return;
        }
        short itemId = pool.get(rand.nextInt(pool.size()));
        Item reward = ItemService.gI().createNewItem(itemId);
        boolean costumeBox = boxId == HOP_CAI_TRANG_HOAI_NIEM;
        int minStat = costumeBox ? 45 : 15;
        int maxStat = costumeBox ? 50 : 20;
        reward.itemOptions.add(new ItemOption(50, Util.nextInt(minStat, maxStat)));
        reward.itemOptions.add(new ItemOption(77, Util.nextInt(minStat, maxStat)));
        reward.itemOptions.add(new ItemOption(103, Util.nextInt(minStat, maxStat)));
        if (costumeBox) {
            reward.itemOptions.add(new ItemOption(210, 4));
        }
        reward.itemOptions.add(new ItemOption(30, 0));
        int days = rollCosmeticMemoryDays();
        if (days > 0) {
            reward.itemOptions.add(new ItemOption(93, days));
        }
        InventoryService.gI().subQuantityItemsBag(player, box, 1);
        InventoryService.gI().addItemBag(player, reward);
        InventoryService.gI().sendItemBag(player);
        Service.gI().sendThongBao(player, "Bạn nhận được " + reward.template.name);
    }

    private List<Short> getCosmeticMemoryPool(Player player, short boxId) {
        List<Short> pool = new ArrayList<>();
        for (Template.ItemTemplate template : Manager.ITEM_TEMPLATES) {
            if (!isValidCosmeticMemoryTemplate(player, template, boxId)) {
                continue;
            }
    ...
    private boolean isValidCosmeticMemoryTemplate(Player player, Template.ItemTemplate template, short boxId) {
        if (template == null || template.name == null || template.name.trim().isEmpty() || template.iconID <= 0) {
            return false;
        }
        if (template.id == HOP_CAI_TRANG_HOAI_NIEM
                || template.id == HOP_PET_VAN_BAY_HOAI_NIEM
                || template.id == HOP_DEO_LUNG_HOAI_NIEM) {
            return false;
        }
        switch (boxId) {
            case HOP_CAI_TRANG_HOAI_NIEM:
                return template.type == 5
                        && (template.gender == 3 || template.gender == player.gender)
                        && !isBlockedCostumeMemoryTemplate(template);
            case HOP_PET_VAN_BAY_HOAI_NIEM:
                return template.type == 21 || template.type == 23 || SPECIAL_MEMORY_PET_IDS.contains((int) template.id);
            case HOP_DEO_LUNG_HOAI_NIEM:
                return template.type == 11 && !isBlockedAccessoryTemplate(template);
            default:
                return false;
        }
    }

    private boolean isBlockedCostumeMemoryTemplate(Template.ItemTemplate template) {
        String name = template.name.toLowerCase();
        return FUSION_COSTUME_TEMPLATE_IDS.contains((int) template.id)
                || BROKEN_COSTUME_MEMORY_TEMPLATE_IDS.contains((int) template.id)
                || !hasValidCostumeParts(template)
                || name.contains("avatar")
                || isSoldInAnyShop(template.id);
    }

    private boolean hasValidCostumeParts(Template.ItemTemplate template) {
        return isPartType(template.head, 0)
                && isPartType(template.body, 1)
                && isPartType(template.leg, 2);
    }

    private boolean isPartType(int partId, int type) {
        Integer partType = Manager.PART_TYPES.get(partId);
        return partType != null && partType == type;
    }

    private boolean isSoldInAnyShop(short templateId) {
    ...
    private void openRadarMemoryBox(Player player, Item box) {
        if (InventoryService.gI().getCountEmptyBag(player) <= 0) {
            Service.gI().sendThongBao(player, "Cần 1 ô trống trong hành trang để mở");
            return;
        }
        List<Short> pool = getRadarMemoryPool(player);
        if (pool.isEmpty()) {
            Service.gI().sendThongBao(player, "Bạn đã sưu tầm đủ các thẻ radar trong hộp này");
            return;
        }
        short itemId = pool.get(rand.nextInt(pool.size()));
        RadarCard radarTemplate = getRadarTemplateByFragmentItemId(itemId);
        int quantity = Util.nextInt(1, 3);
        if (radarTemplate != null) {
            quantity = Math.min(quantity, getRadarCardRemainingToMax(player, radarTemplate));
        }
        Item reward = ItemService.gI().createNewItem(itemId);
        ...
        InventoryService.gI().addItemBag(player, reward);
        InventoryService.gI().sendItemBag(player);
        Service.gI().sendThongBao(player, "Bạn nhận được " + reward.quantity + " " + reward.template.name);
    }

    private List<Short> getRadarMemoryPool(Player player) {
        List<Short> pool = new ArrayList<>();
        for (RadarCard radarTemplate : RadarService.gI().RADAR_TEMPLATE) {
            if (radarTemplate == null || isRadarCardFull(player, radarTemplate)) {
                continue;
            }
            short itemId = getRadarFragmentItemId(radarTemplate.Id);
            Template.ItemTemplate itemTemplate = ItemService.gI().getTemplate(itemId);
            if (itemTemplate == null || itemTemplate.type != 33) {
                continue;
            }
            pool.add(itemId);
        }
    ...
    private boolean isRadarCardFull(Player player, RadarCard radarTemplate) {
        Card card = player.Cards.stream().filter(r -> r != null && r.Id == radarTemplate.Id).findFirst().orElse(null);
        return card != null && card.Level >= getMaxRadarLevel(radarTemplate);
    }

    private short getRadarFragmentItemId(short radarId) {
        return radarId == RADAR_RONG_THAN_NAMEC ? MANH_RONG_THAN_NAMEC : radarId;
    }

    private RadarCard getRadarTemplateByFragmentItemId(short itemId) {
        short radarId = getRadarIdByFragmentItemId(itemId);
        return RadarService.gI().RADAR_TEMPLATE.stream().filter(c -> c.Id == radarId).findFirst().orElse(null);
    }

    private int getRadarCardRemainingToMax(Player player, RadarCard radarTemplate) {
        Card card = player.Cards.stream().filter(r -> r != null && r.Id == radarTemplate.Id).findFirst().orElse(null);
        int maxAmount = Math.max(1, radarTemplate.Max);
        int maxProgress = getMaxRadarLevel(radarTemplate) * maxAmount;
        if (card == null) {
            return maxProgress;
        }
        if (card.Level >= getMaxRadarLevel(radarTemplate)) {
            return 0;
        }
        int progress = Math.max(0, card.Level) * maxAmount + Math.max(0, card.Amount);
        return Math.max(0, maxProgress - progress);
    }

    private boolean isBlockedAccessoryTemplate(Template.ItemTemplate template) {
        if (template.id >= 766 && template.id <= 794) {
            return true;
        }
        if ((template.id >= 353 && template.id <= 360) || template.id == 362 || (template.id >= 372 && template.id <= 378)) {
            return true;
        }
        String description = template.description == null ? "" : template.description.toLowerCase();
        return description.contains("bang");
    }

    private int rollCosmeticMemoryDays() {
    ...
    private void changePetRamdom(Player player, Item item) {
        ...
        if (!item.hasOption(249, 3000)) {
            Service.gI().sendThongBao(player, "Can it nhat 3000 suc manh Kilis de mo!");
            return;
        }
        if (player.pet == null || player.pet.typePet < 1 || player.pet.typePet > 4 || player.pet.nPoint.power < 40_000_000_000L) {
            Service.gI().sendThongBao(player, "Can co de Mabu hoac de Kilis dat 40 ty suc manh de thuc hien!");
            return;
        }
        int[] petTypes = {2, 3, 4};
        int currentType = player.pet.typePet;
        int randomType;
        do {
            randomType = petTypes[rand.nextInt(petTypes.length)];
        } while (currentType >= 2 && currentType <= 4 && randomType == currentType);
        switch (randomType) {
            case 2:
    ...
    private void useItemTime(Player pl, Item item) {
        ...
        switch (item.template.id) {
            ...
            case 1809 -> {
                pl.itemTime.lastTimevevang = now;
                pl.itemTime.isUsevevang = true;
                if (pl.itemTime.totalTimeVeVang <= 0) {
                    pl.itemTime.totalTimeVeVang = ItemTime.TIME_VE_VANG;
                }
            }
            case 1731 -> {
            ...
            case 1628 -> {
                long addTime = getItemTimeMillis(item, ItemTime.TIME_30P);
                long remaining = 0;
                if (pl.itemTime.isUseBuax2DeTu) {
                    remaining = pl.itemTime.totalTimeBuax2DeTu - (now - pl.itemTime.lastTimeBuax2DeTu);
                    if (remaining < 0) {
                        remaining = 0;
                    }
                }
                pl.itemTime.totalTimeBuax2DeTu = remaining + addTime;
    ...
    private long getItemTimeMillis(Item item, long defaultTime) {
        ItemOption timeOption = item.getOptionById(9);
        if (timeOption == null || timeOption.param <= 0) {
            return defaultTime;
        }
        return timeOption.param * 60_000L;
    }
    ...
    private void learnSkill(Player pl, Item item) {
        ...
        try {
            if (item.template.gender == pl.gender || item.template.gender == 3) {
                if (!SkillUtil.isSkillItemForPlayer(pl, item.template.id)) {
                ...
                int currentLevel = curSkill != null ? curSkill.point : 0;
                if (currentLevel == 7) {
                    Service.gI().sendThongBao(pl, "Kỹ năng đã đạt tối đa!");
                } else {
                    if (currentLevel == 0) {
                        if (level == 1) {
                            curSkill = SkillUtil.createSkill(pl, SkillUtil.getTempSkillSkillByItemID(item.template.id), level);
                            ...
                            Skill skillNeed = SkillUtil.createSkill(pl, SkillUtil.getTempSkillSkillByItemID(item.template.id), level);
    ...
    public void UseCard(Player pl, Item item) {
        short radarId = getRadarIdByFragmentItemId(item.template.id);
        RadarCard radarTemplate = RadarService.gI().RADAR_TEMPLATE.stream().filter(c -> c.Id == radarId).findFirst().orElse(null);
        if (radarTemplate == null) return;
        if (radarTemplate.Require != -1) {
            short requireId = radarTemplate.Require;
            RadarCard radarRequireTemplate = RadarService.gI().RADAR_TEMPLATE.stream().filter(r -> r.Id == requireId).findFirst().orElse(null);
        ...
        Card card = pl.Cards.stream().filter(r -> r.Id == radarId).findFirst().orElse(null);
        if (card == null) {
            Card newCard = new Card(radarId, (byte) 1, radarTemplate.Max, (byte) -1, radarTemplate.Options);
    ...
    private short getRadarIdByFragmentItemId(short itemId) {
        return itemId == MANH_RONG_THAN_NAMEC ? RADAR_RONG_THAN_NAMEC : itemId;
    }

    private byte getMaxRadarLevel(RadarCard radarTemplate) {
        byte maxLevel = 2;
        if (radarTemplate != null && radarTemplate.Options != null) {
            for (OptionCard option : radarTemplate.Options) {
                if (option != null && option.active > maxLevel) {
                    maxLevel = option.active;
                }
            }
        }
        return maxLevel;
