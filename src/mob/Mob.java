...
public class Mob {
    ...
    public int getClientIndex() {
        if (this.zone != null && this.zone.mobs != null) {
            int index = this.zone.mobs.indexOf(this);
            if (index >= 0) {
                return index;
            }
        }
...
//            }
            ...
            if (MapService.gI().isMapCadic(this.zone.map.mapId) && plAtt != null
                    && actualDamage >= this.point.maxHp / 10) {
...
//              if (Util.isTrue(successRate, 333)) {
    ...
    public void update() {
        ...
        if (this.isDie() && !isBigBoss()) {
            if (MapService.gI().isMapNguHanhSon(this.zone.map.mapId)) {
    ...
    private void sendMobAttackMe(Player player, long dame) {
        if (!player.isPet && !player.isClone && !player.isNewPet) {
            ...
            try {
                msg = new Message(-11);
                msg.writer().writeByte(this.getClientIndex());
    ...
    private void sendMobAttackPlayer(Player player) {
        ...
        try {
            ...
            msg.writer().writeInt((int) Util.maxIntValue(player.nPoint.hp)); //hp
    ...
    public void sendMobHoiSinh() {
        ...
        try {
            ...
            msg.writer().writeInt((int) Util.maxIntValue(this.point.hp));
    ...
    public void hoi_hp(long hp) {
        ...
        try {
            ...
            msg = new Message(-9);
            msg.writer().writeByte(this.getClientIndex());
            msg.writer().writeInt((int) Util.maxIntValue(this.point.gethp()));
    ...
    private void sendMobDieAffterAttacked(Player plKill, long dameHit) {
        ...
        try {
            msg = new Message(-12);
            msg.writer().writeByte(this.getClientIndex());
            msg.writer().writeInt((int) Util.maxIntValue(dameHit));
    ...
    private void hutItem(Player player, List<ItemMap> items) {
        if (!player.isPet && !player.isBot && !player.isNewPet && !player.isClone) {
            ...
            Player master = player.isClone ? ((PlayerClone) player).master : (player instanceof Pet) ? ((Pet) player).master : null;
            if (master != null && master.charms.tdThuHut > System.currentTimeMillis()) {
                for (ItemMap item : items) {
                    ItemMapService.gI().pickItem(master, item.itemMapId, true);
                }
            }
        }
    }

    private long getDropOwnerId(Player player) {
        if (player instanceof PlayerClone clone && clone.master != null) {
            return clone.master.id;
        }
        if (player instanceof Pet pet && pet.master != null) {
            return pet.master.id;
        }
        return player.id;
    }

    private void normalizeDropOwner(Player player, List<ItemMap> items) {
        long ownerId = getDropOwnerId(player);
        for (ItemMap item : items) {
            if (item != null && item.playerId != -1) {
    ...
    private List<ItemMap> mobReward(Player player, ItemMap itemTask, Message msg) {
        ...
        try {
            ...
            normalizeDropOwner(player, itemReward);
 ...
 /*       if (player.itemTime.isCoBonLa) {
            if (MapService.gI().isMapUpSKH(mapid)) {
                if (Util.isTrue(1, 100)) {
                    ...
                    int[] opsrand = ItemService.gI().randOptionItemKichHoatBalanced(player.gender);
 ...
 */ 
        ...
        if (MapService.gI().isMapUpPorata(mapid)) {
            ...
            if (Util.isTrue(dropRate1, 100)) {
                ...
                list.add(it);
            } else if (Util.isTrue(dropRate2, 10000) && player.itemEvent.canDropManhVo(2000)) {
                ItemMap it = new ItemMap(zone, 934, 1, x, yEnd, player.id);
                it.options.add(new Item.ItemOption(31, 1));
                list.add(it);
            } else if (Util.isTrue(dropRate3, 10000) && player.itemEvent.canDropDaXanhLam(50)) {
        ...
        if (player.actived() ? Util.isTrue(1, 25) : Util.isTrue(1, 100)) {
...
//        }
        ...
        //========================Đồ Sao Khác Vải Thô========================
        if (MapService.gI().isMapUpSKH(mapid)) {
            int denomDoSao = 19999;
            if (player.itemTime.isCoBonLa) {
                denomDoSao = (int) (denomDoSao / 1.15);
            }

            if (Util.isTrue(1, denomDoSao)) {
        ...
        if (MapService.gI().isMapUpSKH(mapid)) {
            ...
            int powerReduction = (int) Math.min(player.nPoint.power / 10_000_000_000L, 5) * 20;
            ...
            if (finalRate > 0 && Util.nextInt(100) < finalRate) {
                ...
                int randOption = Util.nextInt(1, 200);
                boolean hasOption = false;
                if (randOption < 50) {
                    int randAddOption = Util.nextInt(100);
                    if (randAddOption < 60) {
        ...
        if (MapService.gI().isMapCold(mapid)) {
            ...
            int denomDoThan = 50000;
            if (player.itemTime.isCoBonLa) {
                denomDoThan = (int) (denomDoThan / 1.15);
            }
            if (Util.isTrue(1, denomDoThan)) {
        ...
        int denomSetGod = 333;
        if (player.itemTime.isCoBonLa) {
            denomSetGod = (int) (denomSetGod / 1.15);
        }
        if (Util.isTrue(3, denomSetGod)) {
        ...
        if (MapService.gI().isMapCold(mapid)) {
            if (player.isClone) {
                player = ((PlayerClone) player).master;
        ...
        if (Util.isTrue(1, 100000) || (player.actived() && Util.isTrue(1, 10000))) {
            ItemMap it = new ItemMap(zone, 1705, 1, x, yEnd, player.id);
            list.add(it);
        }
        // Manh thien su
        ...
        if (player.actived() ? Util.isTrue(1, 2) : Util.isTrue(1, 100)) {
    ...
    public void sendMobMaxHp(long maxHp) {
        ...
        try {
            msg = new Message(87);
            msg.writer().writeByte(this.getClientIndex());
            msg.writer().writeInt((int) Util.maxIntValue(maxHp));
            Service.gI().sendMessAllPlayerInMap(this.zone, msg);
            msg.cleanup();
        } catch (IOException e) {
        }
    ...
    private void phanSatThuong(Player plTarget, long dame) {
        ...
        if (percentPST != 0) {
            ...
            try {
                ...
                msg.writer().writeInt((int) Util.maxIntValue(this.point.hp));
                msg.writer().writeInt((int) Util.maxIntValue(damePST));
                msg.writer().writeBoolean(false);
