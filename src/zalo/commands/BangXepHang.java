...
public class BangXepHang implements Command {
    ...
    public void run(Command.CommandContext context) {
        try {
            ...
            System.err.println("[BangXepHang] Error: " + e.getMessage());
        }
    }
    
    private String getTitle(String method) {
        switch (method) {
            case "sm":return "Bảng Xếp Hạng Sức Mạnh";
            case "nv":return "Bảng Xếp Hạng Nhiệm Vụ";
            case "whis":return "Bảng Xếp Hạng Whis";
            case "nap":return "Bảng Xếp Hạng Nạp";
    ...
    private String getValueString(String method, JSONObject item) {
        switch (method) {
            case "sm":
                Object powerObj = item.get("power");long power;
                if (powerObj instanceof Long) {
                    power = (Long) powerObj;
                } else if (powerObj instanceof Integer) {
                    power = ((Integer) powerObj).longValue();} else if (powerObj instanceof Number) {
                    power = ((Number) powerObj).longValue();
                } else {
                    power = 0;
                }
                return formatPower(power) + " SM";case "nv":
                Byte nv = ((Long) item.get("nv")).byteValue();
                Byte subnv = ((Long) item.get("subnv")).byteValue();
                return "NV " + nv + "-" + subnv;
            case "whis":
                Integer level = ((Long) item.get("level")).intValue();Integer time = ((Long) item.get("time")).intValue();
