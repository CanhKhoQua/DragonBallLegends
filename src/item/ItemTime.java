...
public class ItemTime {
    ...
    public static final int MAX_ITEM_TIME_SECONDS = Short.MAX_VALUE;
    private static final long REFRESH_LONG_ITEM_TIME_ICON = 60_000L;
    ...
    public long totalTimeBuax2DeTu = TIME_30P;
    public long lastTimeSendBuax2DeTuIcon;
    ...
    public void update() {
        if (isUseKilis) {
            if (Util.canDoWithTime(lastTimeUseKilis, timeLengthKilis)) {
                isUseKilis = false;
            }
        }
        if (isUseLoX2) {
        ...
        if (isUsevevang) {
            long remaining = totalTimeVeVang - (System.currentTimeMillis() - lastTimevevang);
            if (remaining <= 0 || Util.canDoWithTime(lastTimevevang, totalTimeVeVang)) {
                isUsevevang = false;
                totalTimeVeVang = 0;
                Service.gI().point(player);
            }
        ...
        if (isUseBuax2DeTu) {
            long remaining = totalTimeBuax2DeTu - (System.currentTimeMillis() - lastTimeBuax2DeTu);
            if (remaining <= 0 || Util.canDoWithTime(lastTimeBuax2DeTu, totalTimeBuax2DeTu)) {
                isUseBuax2DeTu = false;
                totalTimeBuax2DeTu = TIME_30P;
                lastTimeSendBuax2DeTuIcon = 0;
                ItemTimeService.gI().removeItemTime(player, 13540);
                Service.gI().point(player);
            } else if (remaining / 1000 > MAX_ITEM_TIME_SECONDS
                    && Util.canDoWithTime(lastTimeSendBuax2DeTuIcon, REFRESH_LONG_ITEM_TIME_ICON)) {
                lastTimeSendBuax2DeTuIcon = System.currentTimeMillis();
                ItemTimeService.gI().sendItemTime(player, 13540, MAX_ITEM_TIME_SECONDS);
            }
        }
        if (isEatMeal) {
