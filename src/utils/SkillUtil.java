...
public class SkillUtil {
    ...
    public static Skill createSkill(Player player, int tempId, int level) {
        Skill skill = null;
        SkillTemplate template = findSkillTemplate(player, tempId);
        if (template != null) {
            List<Skill> skills = template.skillss;
            if (skills != null && level >= 1 && level <= skills.size()) {
                skill = skills.get(level - 1);
    ...
    public static SkillTemplate findSkillTemplate(Player player, int tempId) {
        if (player != null && player.gender >= 0 && player.gender < Manager.NCLASS.size()) {
            SkillTemplate template = Manager.NCLASS.get(player.gender).getSkillTemplate(tempId);
            if (template != null) {
                return template;
            }
        }
        return findSkillTemplate(tempId);
    }
...
//            return Skill.SUPER_SAIYAN;
    ...
    public static byte getLevelSkillByItemID(int tempId) {
        if (tempId >= 1824 && tempId <= 1838) {
            return (byte) ((tempId - 1824) % 5 + 1);
        }
        switch (tempId) {
            // TRÁI ĐẤT
            case 1334:
    ...
    public static boolean isSkillItemForPlayer(Player pl, int tempId) {
        if (pl == null) {
            return false;
        }
        if (tempId >= 1824 && tempId <= 1838) {
            return getGenderByEvolutionSkillItem(tempId) == pl.gender;
        }
        return true;
    }

    private static int getGenderByEvolutionSkillItem(int tempId) {
        if (tempId >= 1824 && tempId <= 1828) {
            return 0;
        }
        if (tempId >= 1829 && tempId <= 1833) {
            return 1;
        }
        if (tempId >= 1834 && tempId <= 1838) {
            return 2;
        }
        return -1;
    }
    ...
    public static Skill getSkillByItemID(Player pl, int tempId) {
        if (!isSkillItemForPlayer(pl, tempId)) {
            return null;
        }
