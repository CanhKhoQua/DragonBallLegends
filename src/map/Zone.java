...
public class Zone {
    ...
    public Player getPlayerInMapOffline(Player player, long idPlayer) {
        for (Player pl : bosses) {
            if (pl != null && pl.id == idPlayer && pl.zone == this && pl.nPoint != null
                    && pl instanceof TrainingBoss && ((TrainingBoss) pl).playerAtt == player) {
                return pl;
            }
        }
        return null;
    ...
    public void pickItem(Player player, int itemMapId) {
        ...
        if (itemMap != null && itemMap.itemTemplate != null) {
            ...
            boolean ownerIsPlayer = playerId == player.id || itemMap.playerId == player.id;
            boolean ownerIsClone = player.clone != null && (playerId == player.clone.id || itemMap.playerId == player.clone.id);
            if (ownerIsPlayer || ownerIsClone || itemMap.playerId == -1) {
                                ...
                                switch (item.template.id) {
                                    ...
                                    default -> {
                                        if (item.template.type >= 0 && item.template.type < 5) {
                                            msg.writer().writeUTF(item.template.name + " Ngon hẹ hẹ hẹ!!");
                                        } else {
...
//                }
    ...
    private void infoPlayer(Player plReceive, Player plInfo) {
        ...
        try {
            ...
            msg.writer().writeInt((int) Util.maxIntValue(plInfo.nPoint.hp));
            msg.writer().writeInt((int) Util.maxIntValue(plInfo.nPoint.hpMax));
        ...
        try {
            ...
            try {
                ...
                for (Mob mob : this.mobs) {
                    boolean deadBigBoss = mob.isBigBoss() && mob.tempId != 70 && mob.isDie();
                    long mobHp = deadBigBoss ? 0 : mob.point.gethp();
                    int mobStatus = deadBigBoss ? 0 : mob.status;
                    ...
                    msg.writer().writeInt((int) Util.maxIntValue(mobHp));
                    msg.writer().writeByte(mob.level);
                    msg.writer().writeInt((int) Util.maxIntValue(mob.point.getHpFull()));
                    msg.writer().writeShort(mob.location.x);
                    msg.writer().writeShort(mob.location.y);
                    msg.writer().writeByte(mobStatus);
